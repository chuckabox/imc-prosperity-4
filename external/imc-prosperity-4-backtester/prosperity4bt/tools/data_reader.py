...
class FileSystemReader(BackDataReader):
    ...
    def _read_file_content(self, path_parts: list[str]) -> ContextManager[Optional[Path]]:
        ...
        # Compatibility shim for repositories storing Prosperity data as:
        # "ROUND <n>/data_capsule/prices_round_<n>_day_<d>.csv"
        if len(path_parts) >= 2 and path_parts[0].startswith("round"):
            round_num = path_parts[0].replace("round", "")
            alt_file = self.data_dir.joinpath(f"ROUND {round_num}", "data_capsule", path_parts[-1])
            if alt_file.is_file():
                return wrap_in_context_manager(alt_file)
        return wrap_in_context_manager(None)
