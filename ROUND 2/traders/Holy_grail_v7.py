...
class Trader:
    ...
    # ── Drift watchdog (v13) — protects against non-mean-reverting regimes ────
    # On IMC data |vwap - 10000| stays < 5, so threshold=15 never triggers.
    # On drift scenarios (v_recovery_drift, sideways_drift) this caps osmium
    # losses from −27k down to small losses by abandoning the anchor-fade.
    OSMIUM_DRIFT_THRESHOLD   = 15     # ticks off anchor to count as drifting
    OSMIUM_DRIFT_STREAK_TRIP = 200    # consecutive drift ticks to enter mode
    OSMIUM_DRIFT_FLATTEN_AT  = 25     # in drift mode, flatten earlier
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        # ── Drift watchdog (v13) ─────────────────────────────────────────────
        # If vwap has been far from anchor for too long, stop fading it.
        drift     = vwap_mid - self.OSMIUM_ANCHOR
        abs_drift = abs(drift)
        streak    = self.history["osm_drift_streak"]
        if abs_drift > self.OSMIUM_DRIFT_THRESHOLD:
            streak = min(streak + 1, 1000)
        else:
            streak = max(streak - 2, 0)    # exit faster than enter
        self.history["osm_drift_streak"] = streak
        in_drift = streak >= self.OSMIUM_DRIFT_STREAK_TRIP

        if in_drift:
            # Abandon the anchor — use VWAP as fair so we stop fading a real trend.
            fair = vwap_mid + (0.6 * (1.0 if obi > 0 else -1.0) if abs(obi) >= 0.3 else 0.0)
        ...
        flatten_hard = self.OSMIUM_DRIFT_FLATTEN_AT if in_drift else self.OSMIUM_FLATTEN_HARD
        if pos > flatten_hard and rs > 0:
            # In drift mode, hit the bid to unwind. Normal mode: passive at fair.
            exit_px = bb if in_drift else int(fair)
            flatten_qty = min(pos - self.OSMIUM_FLATTEN_TARGET + 5, rs)
            orders.append(Order(product, exit_px, -flatten_qty))
            rs  -= flatten_qty
            pos -= flatten_qty
        elif pos < -flatten_hard and rb > 0:
            exit_px = ba if in_drift else int(fair)
        ...
        # In drift mode: halve sizes and suppress the side that would add
        # adverse inventory (buying on a downtrend, selling on an uptrend).
