"""
trader_ken_v6.py — Round 2: v5 optimal stack + explicit underwater safety
===========================================================================

Keeps all of ken_v5 (AR(3) Osmium, multi-level VWAP + OFI, linreg Pepper stop,
Pepper crash gate, Titan split trails, mid backfill, bid()=0).

New safety (addresses mark-to-market dips below zero early / mid-session):
  * Global `gl_hwm` trailing: when `gl_pnl` is fragile (< GL_FRAGILE_BELOW) and far
    below session HWM, scale Pepper cap (does not fire on healthy pullbacks at high PnL).
  * Soft floor on `gl_pnl`: extra de-risk while underwater but above hard stop.
  * Boot ramp: first ~360 elapsed ticks use slightly smaller effective caps / takes.
  * EOD cap phases + late no-new-buys only when `gl_pnl` is below GL_EOD_SCHEDULE_BELOW
    (keeps v5-like behavior once the day is working).
  * Pepper cpnl DD shrink only in the near-zero fragile band.
  * Titan / crash thresholds match v5 defaults unless tuned later.
  * Light L1 OBI gate on Pepper (skip passive when book leans sell + loaded).
  * Osmium quote sizes trim when global PnL is negative or off peak.
"""
...
def _obi_l1(depth: OrderDepth) -> float:
    """Top-of-book volume imbalance in [-1, 1] (Holy_grailll-style microstructure)."""
...
class Trader:
    ...
    # ---------------- Global PnL soft rails (combined mark-to-market) ----------------
    GL_BOOT_MAX_ELAPSED = 240
    GL_BOOT_CAP_SCALE = 0.97
    GL_SOFT_FLOOR = -650.0
    GL_STRESS_CAP_SCALE = 0.86
    GL_SOFT_TRAIL = 2200.0
    GL_TRAIL_CAP_SCALE = 0.92
    # Near-zero / underwater band only (do not tag "fragile" during normal early ramp).
    GL_FRAGILE_BELOW = 450.0
    # Apply EOD cap schedule only when global PnL is not deep in profit (v5 had no EOD shrink).
    GL_EOD_SCHEDULE_BELOW = 5200.0

    # ---------------- Pepper schedule risk (chimera-style) ----------------
    PEPPER_EOD_PHASES = ((8800, 0.94), (9300, 0.86), (9700, 0.76), (9950, 0.50))
    PEPPER_EOD_NO_BUY_ELAPSED = 9600
    PEPPER_DD_SHRINK_AT = 2200.0
    PEPPER_DD_CAP_MULT = 0.88

    PEPPER_OBI_SKIP_PASSIVE = -0.28
    PEPPER_OBI_TAKE_BOOST = 1.12
    ...
    CRASH_Z_RESUME = 1.0
    CRASH_MIN_HIST = 30
    CRASH_LOCKOUT_TICKS = 160

    OSMIUM_UNDERWATER_SCALE = 0.90
    OSMIUM_UNDERWATER_GL = -280.0
    OSMIUM_UNDERWATER_TRAIL = 1100.0
    ...
    def _pepper_logic(self, state: TradingState) -> List[Order]:
        ...
        # --- Schedule + global underwater scaling (after regime, before stops) ---
        eod_mult = 1.0
        if gl < self.GL_EOD_SCHEDULE_BELOW:
            for phase_el, mult in self.PEPPER_EOD_PHASES:
                if elapsed >= phase_el:
                    eod_mult = mult

        gl_hwm = self.history.get("gl_hwm", gl)
        fragile = gl < self.GL_FRAGILE_BELOW
        off_peak = (gl_hwm - gl) > self.GL_SOFT_TRAIL and fragile

        risk = 1.0
        if elapsed < self.GL_BOOT_MAX_ELAPSED:
            risk *= self.GL_BOOT_CAP_SCALE
        if gl < self.GL_SOFT_FLOOR:
            risk *= self.GL_STRESS_CAP_SCALE
        if off_peak:
            risk *= self.GL_TRAIL_CAP_SCALE

        pp_cpnl = self.history.get(f"{PEPPER}_cpnl", 0.0)
        pp_hwm = self.history.get(f"{PEPPER}_hwm", 0.0)
        if fragile and (pp_hwm - pp_cpnl) > self.PEPPER_DD_SHRINK_AT:
            risk *= self.PEPPER_DD_CAP_MULT

        effective_cap = int(max(0, effective_cap) * risk * eod_mult)
        eod_no_new_buys = (
            elapsed >= self.PEPPER_EOD_NO_BUY_ELAPSED and gl < self.GL_EOD_SCHEDULE_BELOW
        )

        if regime_cap == self.PEPPER_CAP_STRONG:
            stop_th, resume_th = self.PEPPER_STOP_STRONG, self.PEPPER_RESUME_STRONG
        elif regime_cap == self.PEPPER_CAP_WEAK:
        ...
        if off_peak:
            take_mult *= 0.92
        take_limit = max(8, int(take_limit * take_mult))

        obi = _obi_l1(depth)
        if regime_cap == self.PEPPER_CAP_STRONG and obi > 0.28:
            take_limit = max(8, int(take_limit * self.PEPPER_OBI_TAKE_BOOST))
        ...
        if rem_cap > 0 and not eod_no_new_buys:
            ...
            if rem_cap > 0:
                ...
                skip_passive = obi <= self.PEPPER_OBI_SKIP_PASSIVE and pos > 40
                if passive > 0 and not skip_passive:
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        gl = self.history.get("gl_pnl", 0.0)
        gh = self.history.get("gl_hwm", gl)
        om_fragile = gl < self.GL_FRAGILE_BELOW
        if om_fragile and (
            gl < self.OSMIUM_UNDERWATER_GL
            or (gh - gl) > self.OSMIUM_UNDERWATER_TRAIL
        ):
            om = self.OSMIUM_UNDERWATER_SCALE
            front = max(6, int(front * om))
            second = max(4, int(second * om))
