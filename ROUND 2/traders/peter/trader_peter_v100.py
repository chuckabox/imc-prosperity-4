"""
trader_peter_v100.py — Based on trader_stable_suvin_v2.py
==========================================================
Small targeted improvements over suvin_v2 (no strategy changes, no overfitting):

1. OSMIUM VWAP — multi-level (top 3 per side instead of just best bid/ask)
   Docs explicitly recommend "Utilizing depth across the full order book
   rather than just the first level" for better fair value signal.

2. PEPPER STOP SLOPE — replace raw hist[-1]-hist[-20] with linreg*19
   Same units and thresholds; smoother estimate avoids triggering stops
   on a single-tick noise spike while preserving sensitivity to real drops.

3. OSMIUM FAIR SMOOTHING — extend rolling average from 5 → 10 periods
   Market making benefits from a more stable fair; 5-period average is
   noisy enough to cause unnecessary quote drift in thin-book moments.

All safety guards, caps, limits, and strategy parameters unchanged.
"""
...
class Trader:
    ...
    OSMIUM_VWAP_LEVELS    = 3   # use top N levels per side for VWAP
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        # Docs: "Utilizing depth across the full order book rather than just
        # the first level" yields better signal quality.
