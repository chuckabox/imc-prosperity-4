"""
trader_peter_v3.py — Osmium Flow-Skew + Warmup Build
=====================================================

Pepper Root: unchanged from v2 (spread-alpha exploitation working)

Osmium changes (the alpha):
1. Warmup 100 ticks: observe-only, no quoting → kills early dip from EMA lag
2. Full 3-level VWAP: use entire visible order book (not just top) for fair value
   → fair is more accurate, reduces adversarial fill risk
3. Flow-based quote skew: when buying imbalance active (15-34), lower our ask
   by 1 to capture more sell-side fills into buyers; raise bid by 1 vice versa.
   → Captures asymmetric flow without waiting for position to build up
4. Larger quote sizes: FRONT 28→36, SECOND 20→28
   → More volume through MM = more PnL given we're sole inside-spread MM
5. Faster inventory skew onset: pos=10 (was 15) → rebalance faster

Note on take logic: osmium spread is ~16 units, fair~10000, asks at ~10008.
   ask <= fair-1 = 9999 never fires. Osmium is pure MM. Take logic kept for
   edge cases (unusual price dislocation) but effectively dormant.
"""
...
class Trader:
    ...
    OSMIUM_WARMUP_TICKS = 100       # ticks of observe-only before quoting

    OSMIUM_TOXICITY_THRESHOLD = 35
    OSMIUM_FLOW_SKEW_THRESHOLD = 15  # softer threshold for flow-based quote skew
    OSMIUM_TAKE_EDGE = 1
    OSMIUM_EDGE_POS_STEP = 25

    OSMIUM_SKEW_SOFT  = 10           # faster onset: was 15
    OSMIUM_SKEW_HARD  = 35
    OSMIUM_FLATTEN_HARD = 58
    OSMIUM_FLATTEN_TARGET = 50

    OSMIUM_QUOTE_FRONT  = 36         # was 28
    OSMIUM_QUOTE_SECOND = 28         # was 20
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PEPPER ROOT  (v2 spread-alpha, unchanged)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        # Warmup: build EMA history before quoting to avoid early dip
        os_t0     = self.history.setdefault("os_t0", ts)
        os_warmed = (ts - os_t0) >= self.OSMIUM_WARMUP_TICKS

        # Full 3-level order book VWAP (more accurate than top-of-book only)
        full_vwap = self._full_book_vwap(depth)
        if full_vwap == 0.0:
            full_vwap = mid

        # Blend full-book VWAP toward anchor (prevents runaway fair during gaps)
        ...
        # Flow-based quote skew: lean into active flow direction
        # When buyers are active (not yet toxic), lower ask to sell more into them
        # When sellers are active, raise bid to buy more from them
        flow_skew = 0
        if imbalance >= self.OSMIUM_FLOW_SKEW_THRESHOLD and not toxic_buys:
            flow_skew = 1    # bullish flow: shift quotes down (sell more aggressively)
        elif imbalance <= -self.OSMIUM_FLOW_SKEW_THRESHOLD and not toxic_sells:
            flow_skew = -1   # bearish flow: shift quotes up (buy more aggressively)

        orders: List[Order] = []

        # During warmup: just observe, no orders
        if not os_warmed:
            return orders
        ...
        # ── Liquidity taking (fires only on unusual dislocation) ──────────────
        ...
        # ── Market making ─────────────────────────────────────────────────────
        # Position skew: earlier onset (pos=10) + flow skew on top
        pos_skew  = int(pos / self.OSMIUM_SKEW_SOFT)
        total_skew = pos_skew + flow_skew
