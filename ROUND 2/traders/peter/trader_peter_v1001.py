"""
trader_peter_v1001.py — v1000 + extra smoothing, nothing else
==============================================================
Same strategy and parameters as v1000 (no MAF, Holy_grail tuning). Four
smoothing refinements to reduce jitter in fair value, regime signals, and
quote placement without shifting the underlying logic.

Changes from v1000:

OSMIUM — calmer fair value
--------------------------
1. EMA LAYER: maintain an exponential moving average of fair (α = 0.2,
   ~5-tick half-life) and blend it 50/50 with the current smoothed fair.
   EMA weights recent points more than a simple moving average tail, so it
   responds faster than widening the window while damping single-tick jumps.
2. LONGER WINDOW: simple-MA smoothing window 10 → 15 ticks. Paired with
   larger `op` cap (30 → 60) so the window stays full.

PEPPER — denoised slope input
-----------------------------
3. PRE-SMOOTHED LINREG: the stop-slope linreg now runs on a 3-tick moving
   average of hist[-22:] (produces 20 smoothed points), not on raw mids.
   Cuts high-frequency noise before the regression sees it; the regression
   window and thresholds are unchanged.

OSMIUM — steadier quote prices
------------------------------
4. QUOTE HYSTERESIS: bid/ask prices (bp, ap) only update when they move
   ≥ 1 tick from last emitted; sub-tick drift reuses the prior price.
   Avoids churning the book on fractional fair-value wobble.

All caps, limits, take/passive sizes, circuit-breakers, skew untouched.
"""
...
def _smooth3(vals: list) -> list:
    """3-point centered moving average; returns len(vals)-2 points."""
    if len(vals) < 3:
        return list(vals)
    return [(vals[i - 1] + vals[i] + vals[i + 1]) / 3.0
...
class Trader:
    ...
    # ── v1001 smoothing constants ────────────────────────────────────────────
    OSMIUM_FAIR_MA_WINDOW = 15      # v1000: 10
    OSMIUM_OP_CAP         = 60      # v1000: 30
    OSMIUM_EMA_ALPHA      = 0.2     # ~5-tick half-life
    OSMIUM_EMA_BLEND      = 0.5     # 50/50 blend MA vs EMA
    ...
    def _pepper_logic(self, state: TradingState) -> List[Order]:
        ...
        # v1001: pre-smooth hist before linreg — 3-pt MA over last 22 raw
        # points yields 20 denoised samples, matching the prior window size.
        if len(hist) >= 22:
            smoothed = _smooth3(hist[-22:])  # 20 points
            local_slope = _linreg_slope(smoothed) * (len(smoothed) - 1)
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        if bid_vol + ask_vol > 0:
            ...
            fair += ofi * self.OSMIUM_OFI_MAX_TILT
        ...
        # v1001: longer MA window + EMA blend for extra stability
        w = self.OSMIUM_FAIR_MA_WINDOW
        if len(op) >= w:
            ma = sum(op[-w:]) / w
        elif len(op) >= 5:
            ma = sum(op[-5:]) / 5.0
        ...
        prev_ema = self.history.get("op_ema", fair)
        ema = self.OSMIUM_EMA_ALPHA * fair + (1 - self.OSMIUM_EMA_ALPHA) * prev_ema
        self.history["op_ema"] = ema

        smoothed = self.OSMIUM_EMA_BLEND * ma + (1 - self.OSMIUM_EMA_BLEND) * ema
        fair = 0.6 * fair + 0.4 * smoothed
        ...
        # v1001: quote hysteresis — reuse prior quote when drift < 1 tick
        prev_bp = self.history.get("op_bp")
        prev_ap = self.history.get("op_ap")
        if prev_bp is not None and abs(bp - prev_bp) < 1 and prev_bp < ap:
            bp = prev_bp
        if prev_ap is not None and abs(ap - prev_ap) < 1 and prev_ap > bp:
            ap = prev_ap
        self.history["op_bp"] = bp
        self.history["op_ap"] = ap
