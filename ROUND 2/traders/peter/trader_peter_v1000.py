"""
trader_peter_v1000.py — Holy_grail + docs-driven refinements (NO MAF)
=====================================================================
Base: Holy_grail.py (v8 config: harder Pepper push, proven drift).
This version strips the Market Access Fee bid (no extra-access auction)
and layers in improvements explicitly called out in the Round 2 wiki.

Changes from Holy_grail.py:

NO MARKET ACCESS FEE
--------------------
• `bid()` removed entirely. No 15k fee paid, no reliance on +25% extra
  quote flow. Strategy runs on the standard 80% volume allocation.

OSMIUM — richer fair value (docs §4)
------------------------------------
1. MULTI-LEVEL VWAP: top 3 price levels per side, not just best bid/ask.
   Wiki: "Utilizing depth across the full order book rather than just the
   first level" improves signal quality.
2. ORDER-FLOW IMBALANCE (OFI) TILT: small ±0.5 nudge to fair based on
   top-of-book volume imbalance. Wiki: "Adjusting price signals based on
   significant imbalance periods."
3. SMOOTHER FAIR: rolling average window 5 → 10 ticks. Calmer MM quotes,
   fewer unnecessary cancel/replace cycles from thin-book noise.

PEPPER — sturdier stop detection (docs §6 — Time-Series Memory)
----------------------------------------------------------------
4. LINREG STOP SLOPE: local slope uses OLS over last 20 points (× 19 to
   keep identical units and thresholds) instead of raw hist[-1]−hist[-20].
   Eliminates false stops triggered by single-tick noise spikes.

BOTH — gap handling (docs §6, pillar 1)
---------------------------------------
5. MID-PRICE BACKFILL: when either side of the book is empty, the history
   now carries forward the last known mid so time-series state (slopes,
   base samples, regime cap, elapsed counters) stays coherent across gaps
   instead of freezing mid-computation.

All caps, limits, circuit-breakers, skew, and take/passive thresholds
preserved from Holy_grail.
"""
...
class Trader:
    ...
    # ── PEPPER constants (from Holy_grail) ───────────────────────────────────
    ...
    # ── OSMIUM constants (from Holy_grail) ───────────────────────────────────
    ...
    # ── v1000 constants ──────────────────────────────────────────────────────
    OSMIUM_VWAP_LEVELS    = 3     # depth levels per side for VWAP
    OSMIUM_OFI_MAX_TILT   = 0.5   # max fair tilt from book imbalance
    ...
    def _pepper_logic(self, state: TradingState) -> List[Order]:
        ...
        # v1000: mid-price backfill when book is incomplete — keep hist coherent
        ...
        if len(hist) >= 20:
            # v1000: linreg × 19 ≈ hist[-1]−hist[-20] but smoother
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        # v1000: mid-price backfill for coherent time-series
        if not depth.buy_orders or not depth.sell_orders:
            op = self.history["op"]
            if op:
                op.append(op[-1])
        ...
        # ── v1000: multi-level VWAP (top N levels per side) ──────────────────
        ...
        # ── v1000: OFI tilt — subtle nudge from order book imbalance ─────────
        ...
        # ── v1000: extend smoothing 5 → 10 periods for calmer MM quotes ──────
        if len(op) >= 10:
