"""
trader_v10100_test.py — Higher aggression test.
"""
import json
from typing import Dict, List
from datamodel import Order, OrderDepth, TradingState, Symbol

# (Same helper functions as v10000)
...
class Trader:
    LIMIT = 80
    PEPPER_WARMUP_TICKS = 1200; PEPPER_FAST_TRACK_TICKS = 200
    PEPPER_SLOPE_STRONG_FAST = 0.05; PEPPER_SLOPE_STRONG = 0.04
    PEPPER_SLOPE_MODERATE = 0.01; PEPPER_SLOPE_WEAK = -0.01
    PEPPER_CAP_STRONG = 80; PEPPER_CAP_MODERATE = 60
    PEPPER_CAP_WEAK = 30; PEPPER_CAP_NEGATIVE = 0; PEPPER_CAP_TENTATIVE = 25
    
    # AGGRESSION BUMP
    PEPPER_TAKE_STRONG = 45 # was 32
    PEPPER_TAKE_NORMAL = 22 # was 18
    PEPPER_PASSIVE_MAX = 70 # was 65
    
    PEPPER_STOP_BREACH_COUNT = 3
    PEPPER_STOP_STRONG = -25 # was -20
    PEPPER_STOP_MODERATE = -12 # was -10
    PEPPER_STOP_WEAK = -8 # was -7
    PEPPER_RESUME_STRONG = 5; PEPPER_RESUME_MODERATE = 5; PEPPER_RESUME_WEAK = 4
    PEPPER_SPREAD_PASSIVE_SCALE = 0.75; PEPPER_TAKE_CROSS_EDGE = 2.0
    PEPPER_OBI_STRONG = 0.30; PEPPER_OBI_TAKE_BOOST = 1.6; PEPPER_OBI_PASSIVE_BOOST = 1.25

    OSMIUM_ANCHOR = 10_000; OSMIUM_TOXICITY_THRESHOLD = 35
    OSMIUM_TAKE_EDGE = 0; OSMIUM_EDGE_POS_STEP = 30
    OSMIUM_TAKE_EDGE_MAX = 5 # was 3
    OSMIUM_SKEW_SOFT = 15; OSMIUM_SKEW_HARD = 35
    OSMIUM_FLATTEN_HARD = 58; OSMIUM_FLATTEN_TARGET = 50
    
    # MM AGGRESSION BUMP 
    OSMIUM_QUOTE_FRONT = 45 # was 38
    OSMIUM_QUOTE_SECOND = 35 # was 28
    
    OSMIUM_SPREAD_CLAMP = 5; OSMIUM_VWAP_WEIGHT = 0.80; OSMIUM_DRIFT_SCALE_AT = 8
    OSMIUM_OBI_STRONG = 0.30; OSMIUM_OBI_FAIR_SHIFT = 0.8; OSMIUM_OBI_SIZE_BOOST = 1.25
    OSMIUM_VWAP_LEVELS = 3; OSMIUM_FAIR_MA_WIN = 10; OSMIUM_OP_CAP = 50; OBI_SMOOTH_WINDOW = 3
    ...
    def _load_state(self, state: TradingState):
        ...
        for k in ["pp", "pp_base", "op", "pp_obi_hist", "op_obi_hist"]: self.history.setdefault(k, [])
    def _save_state(self) -> str: return json.dumps(self.history)
    @staticmethod
    def _push_smooth(h, v, w):
        h.append(v)
        if len(h) > w: del h[:len(h)-w]
        return sum(h)/len(h)
    ...
    def _pepper_logic(self, state: TradingState) -> List[Order]:
        ...
        if not d.buy_orders or not d.sell_orders:
            h = self.history["pp"]; 
            if h: h.append(h[-1]); self.history["pp"] = h[-120:]
            return []
        bb = max(d.buy_orders.keys()); ba = min(d.sell_orders.keys()); mid = (bb+ba)/2.0; spr = ba-bb; ts = state.timestamp
        obi_r = _obi_l1(d); obi = self._push_smooth(self.history["pp_obi_hist"], obi_r, self.OBI_SMOOTH_WINDOW)
        h = self.history["pp"]; h.append(mid); self.history["pp"] = h[-120:]
        bs = self.history["pp_base"]
        if len(bs) < 15: bs.append(mid)
        t0 = self.history.setdefault("pp_t0", ts); elap = ts - t0; wup = elap >= self.PEPPER_WARMUP_TICKS; ftrack = elap >= self.PEPPER_FAST_TRACK_TICKS
        ps = self.history.get("pp_prev_spread", spr); wid = spr > ps; self.history["pp_prev_spread"] = spr
        cap = self.history.get("pp_cap", None)
        if len(bs) >= 15 and len(h) >= 15:
            bm = _median(bs); cm = _median(h[-15:]); drift = (cm-bm)/max(1, elap)*100.0
            if ftrack and drift >= self.PEPPER_SLOPE_STRONG_FAST: ncap = self.PEPPER_CAP_STRONG
            elif wup:
            ...
            if cap is None: cap = ncap if ncap is not None else self.PEPPER_CAP_TENTATIVE
            elif ncap is not None:
                if ncap > cap and obi < -0.5: pass
                else: cap = ncap
        ...
        bc = int(self.history.get("pp_breach", 0)); stopd = bool(self.history.get("pp_stopped", False))
        if len(h) >= 20:
            ls = _linreg_slope(h[-20:])*19
            if ls < sth: bc += 1
            else: bc = 0
            if bc >= self.PEPPER_STOP_BREACH_COUNT: stopd = True
            elif stopd and ls > rth: stopd = False
        ...
        if stopd or ecap == 0:
            if pos > 0:
                ...
                if q > 0: orders.append(Order(p, bb, -q))
            return orders
        rcap = ecap - pos; tlim = self.PEPPER_TAKE_STRONG if ecap == self.PEPPER_CAP_STRONG else self.PEPPER_TAKE_NORMAL
        if obi >= self.PEPPER_OBI_STRONG: tlim = int(tlim * self.PEPPER_OBI_TAKE_BOOST)
        if rcap > 0:
            bud = min(rcap, tlim)
    ...
    def _osmium_logic(self, state: TradingState) -> List[Order]:
        ...
        if not d.buy_orders or not d.sell_orders:
            op = self.history["op"]; 
            if op: op.append(op[-1]); self.history["op"] = op[-self.OSMIUM_OP_CAP:]
            return []
        ...
        op = self.history["op"]; op.append(fair); self.history["op"] = op[-self.OSMIUM_OP_CAP:]
        w = self.OSMIUM_FAIR_MA_WIN
        if len(op) >= w: fair = 0.6*fair + 0.4*(sum(op[-w:])/w)
        ...
        sk = int(pos / self.OSMIUM_SKEW_SOFT)
        bp = int(min(bb+1, fair-1)) - sk; ap = int(max(ba-1, fair+1)) - sk
        if obi_c >= self.OSMIUM_OBI_STRONG: bp += 1
        elif obi_c <= -self.OSMIUM_OBI_STRONG: ap -= 1
        bp = max(bp, int(fair)-self.OSMIUM_SPREAD_CLAMP); ap = min(ap, int(fair)+self.OSMIUM_SPREAD_CLAMP)
        ...
        if rb > 0 and not tbuys:
            ...
            if rb > 0: orders.append(Order(p, bp-1, min(rb, sb)))
        if rs > 0 and not tsells:
            q = min(rs, fa); orders.append(Order(p, ap, -q)); rs -= q
            if rs > 0: orders.append(Order(p, ap+1, -min(rs, sa)))
        return orders
