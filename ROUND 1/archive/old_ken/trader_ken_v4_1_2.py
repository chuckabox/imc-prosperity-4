...
class Trader:
    """
    trader_ken v4.1.2
    Tiny refinement of v4.1:
    - slightly more responsive Osmium fair
    - keep Pepper baseline unchanged
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        tape_adj = math.copysign(min(abs(tape_volume) * 0.188, 3.05), tape_volume)
        mid_pull = max(-0.95, min(0.95, (mid - 10000.0) * 0.145))
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            take_margin = 2.33 if spread <= 2 else 2.73
            if abs(position) >= 55:
                take_margin += 0.12
