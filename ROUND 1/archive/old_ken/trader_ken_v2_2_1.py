...
class Trader:
    """
    trader_ken v2.2.1 hotfix
    - Osmium: unchanged from proven baseline
    - Pepper: minimal-delta adaptive unwind only
    """
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Keep baseline aggressive accumulation when not heavily long.
            max_take = buy_cap if position < 50 else min(buy_cap, 25)
            taken = 0
            for ask in sorted(depth.sell_orders.keys()):
                if taken >= max_take:
                    break
                qty = min(abs(depth.sell_orders[ask]), max_take - taken)
                if qty <= 0:
                    break
                orders.append(Order(product, ask, qty))
                buy_cap -= qty
            ...
            if buy_cap > 0 and depth.buy_orders and position < 60:
                best_bid = max(depth.buy_orders.keys())
                rest_qty = buy_cap if position < 45 else min(buy_cap, 15)
                if rest_qty > 0:
                    orders.append(Order(product, best_bid + 1, rest_qty))

            # Adaptive exit threshold: unload faster when inventory gets heavy.
            exit_buffer = 3.0
            if position >= 45:
                exit_buffer = 2.1
            if position >= 65:
