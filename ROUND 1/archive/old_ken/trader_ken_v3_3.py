...
class Trader:
    """
    trader_ken v3.3
    - Osmium: enhanced microstructure execution
    - Pepper: unchanged v2 baseline logic
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        # Slightly stronger tape response, anchored around 10k.
        tape_adj = math.copysign(min(abs(tape_volume) * 0.18, 3.0), tape_volume)
        # Small mid pull to react to local imbalance while staying anchored.
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Dynamic take margin based on spread and inventory.
            take_margin = 2.3 if spread <= 2 else 2.7
            if abs(position) >= 50:
                take_margin += 0.2
            ...
            # Dynamic skew: stronger flattening when inventory is high.
            skew_factor = 0.05 if abs(position) < 45 else 0.08
            ...
            # Two-level maker quoting for queue depth diversity.
            if rem_buy > 0:
                top_buy = min(rem_buy, math.ceil(rem_buy * 0.65))
            ...
            if rem_sell > 0:
                top_sell = min(rem_sell, math.ceil(rem_sell * 0.65))
        ...
        # ---- PEPPER unchanged baseline from v2 ----
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
