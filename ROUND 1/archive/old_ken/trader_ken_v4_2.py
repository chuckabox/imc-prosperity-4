...
class Trader:
    ...
    trader_ken v4.2
    - Derived from v3.3
    - Osmium slightly more aggressive in tight spreads
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        tape_adj = math.copysign(min(abs(tape_volume) * 0.185, 3.1), tape_volume)
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Slightly more aggressive than v3.3 in tight spread.
            take_margin = 2.2 if spread <= 2 else 2.7
            if abs(position) >= 50:
                take_margin += 0.15
            ...
            skew_factor = 0.05 if abs(position) < 45 else 0.09
            ...
            if rem_buy > 0:
                top_buy = min(rem_buy, math.ceil(rem_buy * 0.70))
            ...
            if rem_sell > 0:
                top_sell = min(rem_sell, math.ceil(rem_sell * 0.70))
