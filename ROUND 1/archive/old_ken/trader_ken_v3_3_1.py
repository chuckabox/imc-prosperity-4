...
class Trader:
    """
    trader_ken v3.3.1
    Tiny refinement of v3.3:
    - same structure
    - small Osmium parameter nudges only
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        tape_adj = math.copysign(min(abs(tape_volume) * 0.19, 3.2), tape_volume)
        mid_pull = max(-1.2, min(1.2, (mid - 10000.0) * 0.20))
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            take_margin = 2.2 if spread <= 2 else 2.6
