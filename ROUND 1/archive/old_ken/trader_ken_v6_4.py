...
class Trader:
    """
    trader_ken v6.4
    - Osmium: keep strong v6.1 profile
    - Pepper: add selective taking + inventory-targeted quoting
    """
    ...
    def run(self, state: TradingState):
        ...
        # OSMIUM (keep proven v6.1 behavior)
        product = "ASH_COATED_OSMIUM"
        ...
        # PEPPER (selective taking with regime-aware inventory target)
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
            ...
            # Take only when there is clear edge and in the direction of target.
            for ask, vol in sorted(depth.sell_orders.items()):
                if buy_cap <= 0:
                    break
                if ask <= fair - buy_edge and position < target + 25:
                    qty = min(abs(vol), buy_cap, 24)
                    orders.append(Order(product, ask, qty))
                    buy_cap -= qty
                    position += qty
                else:
            ...
            if buy_cap > 0:
                buy_qty = min(buy_cap, 28 if gap > 0 else 14)
                if buy_qty > 0:
                    orders.append(Order(product, passive_bid, buy_qty))

            if sell_cap > 0:
                sell_qty = min(sell_cap, 28 if gap < 0 else 14)
                if sell_qty > 0:
