...
class Trader:
    """
    trader_ken v2.2 hotfix:
    - Keep aggressive entries
    - Add stronger position management on Pepper
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        _best_bid = max(depth.buy_orders.keys()) if depth.buy_orders else 10000
        _best_ask = min(depth.sell_orders.keys()) if depth.sell_orders else 10000
        ...
        tape_adj = math.copysign(min(abs(tape_volume) * 0.17, 2.7), tape_volume)
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Keep aggressive behavior, but when already long, slow additional buys.
            max_follow_size = 80 if position < 40 else 20
            bought = 0
            for ask in sorted(depth.sell_orders.keys()):
                if buy_cap <= 0 or bought >= max_follow_size:
                    break
                qty = min(abs(depth.sell_orders[ask]), buy_cap, max_follow_size - bought)
                orders.append(Order(product, ask, qty))
                buy_cap -= qty
                bought += qty

            if buy_cap > 0 and depth.buy_orders and position < 55:
                best_bid = max(depth.buy_orders.keys())
                orders.append(Order(product, best_bid + 1, min(buy_cap, 30)))

            # Adaptive exits: unload faster when inventory is heavy.
            take_profit = 3.0
            if position >= 40:
                take_profit = 2.0
            if position >= 60:
