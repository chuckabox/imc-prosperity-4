...
class Trader:
    """
    trader_ken v6.2
    Alternate search branch from v3.3 family:
    - Slightly more conservative take, stronger top-quote size
    - Pepper unchanged baseline
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        tape_adj = math.copysign(min(abs(tape_volume) * 0.182, 2.95), tape_volume)
        mid_pull = max(-0.95, min(0.95, (mid - 10000.0) * 0.13))
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            take_margin = 2.3 if spread <= 2 else 2.75
            if abs(position) >= 50:
                take_margin += 0.15
            ...
            if rem_buy > 0:
                top_buy = min(rem_buy, math.ceil(rem_buy * 0.72))
            ...
            if rem_sell > 0:
                top_sell = min(rem_sell, math.ceil(rem_sell * 0.72))
