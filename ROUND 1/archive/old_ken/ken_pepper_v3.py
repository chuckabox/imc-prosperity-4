...
class Trader:
    ...
    - Pepper: original aggressive logic + minimal near-limit safety unwind
    """
    ...
    def run(self, state: TradingState):
        ...
        # PEPPER: original aggressive + tiny safety adjustment
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
            ...
            # Keep original greedy asks, but reduce add-on when already stretched long.
            buy_clip_cap = 80 if position < 65 else 8
            ...
            if buy_cap > 0 and depth.buy_orders:
                ...
                rest_bid_qty = buy_cap if position < 70 else min(buy_cap, 5)
                if rest_bid_qty > 0:
                    orders.append(Order(product, best_bid + 1, rest_bid_qty))

            # Original spike exit with emergency near-limit de-risk.
            near_limit = abs(position) >= 70
            spike_edge = 2.0 if near_limit else 3.0
