...
class Trader:
    ...
    trader_ken v4.1.3
    - Osmium same as v4.1
    - Pepper: adaptive exit threshold and reduced over-accumulation
    """
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        mid_pull = max(-0.9, min(0.9, (mid - 10000.0) * 0.14))
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            take_margin = 2.35 if spread <= 2 else 2.75
            if abs(position) >= 55:
                take_margin += 0.15
        ...
        if product in state.order_depths:
            ...
            # Still aggressive, but taper resting adds when heavily long.
            ...
            if buy_cap > 0 and depth.buy_orders:
                ...
                rest_qty = buy_cap if position < 55 else min(buy_cap, 8)
                if rest_qty > 0:
                    orders.append(Order(product, best_bid + 1, rest_qty))

            # Adaptive exit threshold for faster de-risking near upper inventory.
            exit_buffer = 3.0
            if position >= 45:
                exit_buffer = 2.3
            if position >= 65:
