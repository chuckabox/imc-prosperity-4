...
class Trader:
    ...
    trader_ken v4.1.4
    - Osmium same as v4.1
    - Pepper: adds tactical rich-bid fade while preserving base behavior
    """
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Tactical extra fade at rich prices if already long.
            if position >= 35 and sell_cap > 0:
                for bid in sorted(depth.buy_orders.keys(), reverse=True):
                    if bid >= fair + 4.4 and sell_cap > 0:
                        qty = min(abs(depth.buy_orders[bid]), sell_cap, 12)
                        orders.append(Order(product, bid, -qty))
                        sell_cap -= qty
                    else:
                        break
