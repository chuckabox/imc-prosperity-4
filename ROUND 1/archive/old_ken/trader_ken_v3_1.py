...
class Trader:
    """
    trader_ken v3.1
    Minimal step from v2:
    - Keep Osmium unchanged
    - Keep Pepper aggressive entries
    - Add lightweight adaptive exit pressure on large Pepper inventory
    """
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Preserve baseline aggressive accumulation.
            ...
            if buy_cap > 0 and depth.buy_orders:
                ...
                # Slightly reduce additional resting buy when already heavy long.
                rest_buy_qty = buy_cap if position < 55 else min(buy_cap, 12)
                if rest_buy_qty > 0:
                    orders.append(Order(product, best_bid + 1, rest_buy_qty))

            # Adaptive exit threshold: unload sooner when position is large.
            exit_buffer = 3.0
            if position >= 45:
                exit_buffer = 2.2
            if position >= 65:
