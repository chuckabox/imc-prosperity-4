...
class Trader:
    """
    ken_pepper_v1
    - Osmium: keep v6.6 baseline behavior
    - Pepper: balanced safety variant (between v6.6 and v6.7)
    """
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            for ask, vol in sorted(depth.sell_orders.items()):
                if buy_cap <= 0:
                    break
                if ask <= fair - buy_edge and position < target + 16:
                    qty = min(abs(vol), buy_cap, 16)
                    orders.append(Order(product, ask, qty))
                    buy_cap -= qty
                    position += qty
                else:
            ...
            for bid, vol in sorted(depth.buy_orders.items(), reverse=True):
                if sell_cap <= 0:
                    break
                if bid >= fair + sell_edge and position > target - 16:
                    qty = min(abs(vol), sell_cap, 16)
                    orders.append(Order(product, bid, -qty))
                    sell_cap -= qty
                    position -= qty
                else:
            ...
            gap = target - position
            bias = max(-1.8, min(1.8, gap / 15.0))

            passive_bid = min(best_bid + 1, int(math.floor(fair - 1.35 + bias)))
            passive_ask = max(best_ask - 1, int(math.ceil(fair + 1.35 + bias)))
