...
class Trader:
    ...
    - Pepper: original aggressive v2/10k behavior for comparison
    """
    ...
    def run(self, state: TradingState):
        ...
        # PEPPER: original aggressive v2/10k logic
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
            ...
            # Resting bid above top of book if we still have buy room.
            if buy_cap > 0 and depth.buy_orders:
                best_bid = max(depth.buy_orders.keys())
                orders.append(Order(product, best_bid + 1, buy_cap))

            # Sell only on sharp spikes above fair.
