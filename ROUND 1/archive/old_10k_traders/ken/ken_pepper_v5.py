...
class Trader:
    ...
    - Pepper: two-mode design
      * normal mode: original aggressive behavior
      * protection mode: activates only under stress and prioritizes unwind
    """
    ...
    def get_pepper_fair(self, state: TradingState) -> float:
        ...
        # Slightly reduced confluence boost for smoother stress transitions.
        if momentum != 0:
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            adverse = mid < fair - 2.0 if position > 0 else (mid > fair + 2.0 if position < 0 else False)

            # Stress trigger: large inventory + adverse move.
            protection_mode = abs(position) >= 62 or (abs(position) >= 50 and adverse)
            ...
            if not protection_mode:
                ...
                # Protection mode: prioritize unloading stretched inventory.
                if position > 0:
                    unwind_edge = 1.3 if position >= 70 else 1.7
                    for bid in sorted(depth.buy_orders.keys(), reverse=True):
                        if sell_cap <= 0:
                            break
                        if bid >= fair + unwind_edge:
                            ...
                            break
                    # Avoid adding long unless very cheap.
                    for ask in sorted(depth.sell_orders.keys()):
                        if buy_cap <= 0:
                            break
                        if ask <= fair - 2.6 and position < 72:
                            qty = min(abs(depth.sell_orders[ask]), buy_cap, 4)
                            orders.append(Order(product, ask, qty))
                            buy_cap -= qty
                            position += qty
                        else:
                    ...
                    for ask in sorted(depth.sell_orders.keys()):
                        ...
                        if ask <= fair - unwind_edge:
                            qty = min(abs(depth.sell_orders[ask]), buy_cap, 30)
                            orders.append(Order(product, ask, qty))
                            buy_cap -= qty
                            position += qty
                        else:
                    ...
                    for bid in sorted(depth.buy_orders.keys(), reverse=True):
                        if sell_cap <= 0:
                            break
                        if bid >= fair + 2.6 and position > -72:
                            qty = min(abs(depth.buy_orders[bid]), sell_cap, 4)
                            orders.append(Order(product, bid, -qty))
                            sell_cap -= qty
                            position -= qty
                        else:
                ...
                # Keep one small passive order in unwind direction.
                if position > 0 and sell_cap > 0:
                    orders.append(Order(product, max(best_ask - 1, int(math.ceil(fair + 1.0))), -min(8, sell_cap)))
                elif position < 0 and buy_cap > 0:
                    orders.append(Order(product, min(best_bid + 1, int(math.floor(fair - 1.0))), min(8, buy_cap)))
