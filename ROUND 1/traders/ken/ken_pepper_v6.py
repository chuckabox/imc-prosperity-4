...
class Trader:
    ...
    - Pepper: original aggressive core + high-threshold safety rails
    """
    ...
    def run(self, state: TradingState):
        ...
        # PEPPER: original core + high-threshold safeguards
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
            ...
            adverse_long = position > 0 and mid < fair - 2.2
            adverse_short = position < 0 and mid > fair + 2.2

            # Protection mode should be rare; only near hard limits or stretched + adverse.
            protection_mode = abs(position) >= 74 or (abs(position) >= 68 and (adverse_long or adverse_short))
            self.history["pepper_mode"] = "protection" if protection_mode else "normal"

            # Original aggressive accumulation with mild taper only when quite stretched.
            buy_clip = 80 if position < 70 else 8
            for ask in sorted(depth.sell_orders.keys()):
                if buy_cap <= 0:
                    break
                qty = min(abs(depth.sell_orders[ask]), buy_cap, buy_clip)
                if qty > 0:
                    orders.append(Order(product, ask, qty))
                    buy_cap -= qty
                    position += qty
            ...
            if buy_cap > 0 and depth.buy_orders:
                rest_qty = buy_cap if position < 72 else min(buy_cap, 4)
                if rest_qty > 0:
                    orders.append(Order(product, best_bid + 1, rest_qty))

            # Exits: keep original in normal mode; become more eager only in protection mode.
            spike_edge = 3.0 if not protection_mode else 2.2
            if position >= 75:
                spike_edge = 1.8
            if position <= -75:
                spike_edge = 1.8
            ...
            for bid in sorted(depth.buy_orders.keys(), reverse=True):
                if bid > fair + spike_edge and sell_cap > 0:
                    max_clip = 80 if not protection_mode else 26
                    qty = min(abs(depth.buy_orders[bid]), sell_cap, max_clip)
                    orders.append(Order(product, bid, -qty))
                    sell_cap -= qty
                    position -= qty
                else:
            ...
            # Short-side symmetry protection.
            if position <= -74:
                for ask in sorted(depth.sell_orders.keys()):
                    if buy_cap <= 0:
                        break
                    if ask < fair - 1.8:
                        qty = min(abs(depth.sell_orders[ask]), buy_cap, 26)
                        orders.append(Order(product, ask, qty))
                        buy_cap -= qty
                        position += qty
                    else:
