...
class Trader:
    ...
    trader_ken v6.7
    - Osmium: same as v6.6
    - Pepper: safer inventory and tighter risk caps
    """
    ...
    def run(self, state: TradingState):
        ...
        if product in state.order_depths:
            ...
            # Smaller directional target than v6.6 to reduce carry risk.
            target = 0
            if regime == "up":
                target = 18
            elif regime == "down":
            ...
            # Widen neutral edges; keep directional asymmetry in trend.
            buy_edge = 1.4
            sell_edge = 1.4
            if regime == "up":
                buy_edge = 0.8
            ...
            # Hard safety rails near limits: only trade to reduce exposure.
            if position >= 65:
                buy_cap = 0
            if position <= -65:
                sell_cap = 0
            ...
            for ask, vol in sorted(depth.sell_orders.items()):
                if buy_cap <= 0:
                    break
                if ask <= fair - buy_edge and position < target + 14:
                    qty = min(abs(vol), buy_cap, 14)
                    orders.append(Order(product, ask, qty))
                    buy_cap -= qty
                    position += qty
                else:
            ...
            for bid, vol in sorted(depth.buy_orders.items(), reverse=True):
                if sell_cap <= 0:
                    break
                if bid >= fair + sell_edge and position > target - 14:
                    qty = min(abs(vol), sell_cap, 14)
                    orders.append(Order(product, bid, -qty))
                    sell_cap -= qty
                    position -= qty
                else:
            ...
            gap = target - position
            bias = max(-1.6, min(1.6, gap / 14.0))

            # Slightly wider passive quotes and smaller clip sizes for safety.
            passive_bid = min(best_bid + 1, int(math.floor(fair - 1.5 + bias)))
            passive_ask = max(best_ask - 1, int(math.ceil(fair + 1.5 + bias)))

            if buy_cap > 0:
                buy_qty = min(buy_cap, 20 if gap > 0 else 8)
                if position >= 55:
            ...
            if sell_cap > 0:
                sell_qty = min(sell_cap, 20 if gap < 0 else 8)
                if position <= -55:
