...
class Trader:
    """
    trader_ken v4.1
    Built from v3.3 family (best local profile), with live-oriented micro tweaks.
    """
    ...
    def run(self, state: TradingState):
        ...
        # ---- OSMIUM (v3.3 style + tiny live tweaks) ----
        product = "ASH_COATED_OSMIUM"
        if product in state.order_depths:
            ...
            # Slightly more conservative taker edges for live robustness.
        ...
        # ---- PEPPER baseline behavior (unchanged) ----
        product = "INTARIAN_PEPPER_ROOT"
        if product in state.order_depths:
            depth = state.order_depths[product]
            position = state.position.get(product, 0)
