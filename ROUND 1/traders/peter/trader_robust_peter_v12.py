"""
trader_robust_peter_v12.py
==========================
V12 — v11 evolved. Same "Aggressive by Default, Safe by Exception" thesis,
six targeted upgrades, each addressing a concrete leak in v11.

Pepper (upside)
---------------
1. SEPARATE take and passive budgets (BUG-FIX of v11's intent).
   v11 set PASSIVE_CAP=40 to accumulate 4x faster — but
     tick_budget = min(rem_cap, add_per_tick=10)
   meant the passive cap never actually bound. v12 has:
     take budget   = PEPPER_TAKE_PER_TICK      (10 normal / 15 strong)
     passive order = PEPPER_PASSIVE_CAP (40)   — independent of take
   so we now genuinely rest a 40-lot bid while still taking up to 15.

2. STRONG regime gets PEPPER_TAKE_PER_TICK_STRONG = 15 (was 10).
   At cap 80, fills in ~6 ticks vs 8. Moves with the drift faster.

3. FAST-TRACK cap commit at ~700 ts (was only after full 1500-ts warmup).
   If smoothed slope already >= PEPPER_SLOPE_FASTTRACK (0.10 — much
   stronger than STRONG=0.06), commit cap 80 immediately. Early drift
   days stop "wasting" the warmup at tentative cap 20.

4. FIRST-TICK-OF-STOP flatten boost: dump 30 the moment the stop fires
   (highest-confidence adverse signal), then 15/tick after. Cuts the
   exposure window after a real reversal.

Osmium (downside)
-----------------
5. DIRECTIONAL toxic-flow gate. If buyers >> sellers on the tape we
   skip our BUYS (don't lift their lifted asks) but keep our SELLS
   (hit their panicky bids). Symmetric for seller-dominant flow.
   v11 blocked both sides and lost the profitable leg.

6. ADAPTIVE take-edge when anchor is drifting. Normal = 1. When the
   20-tick median is > 6 off the 10k anchor we raise take-edge to 2.
   If fair is uncertain, demand more edge before crossing.
"""
...
class Trader:
    ...
    def _pepper_logic(self, state: TradingState) -> List[Order]:
        ...
        # Fast-track: on obviously strong drift, commit early.
        if (
            cap is None
