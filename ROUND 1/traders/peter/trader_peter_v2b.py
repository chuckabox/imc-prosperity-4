...
class Trader:
    """
    trader_peter_v2b: The Adaptive Champion.
    ---------------------------------------
    Updates trader_peter_v2 with an 'Adaptive Anchor' system for Osmium.
    Instead of assuming a fixed 10,000.0 peg, it tracks a long-term EMA (alpha=0.01)
    to adjust for market regime shifts while retaining micro-alpha from tape and OFI.
    """
    def __init__(self):
        self.limits = {
    ...
    def _ema(self, symbol: str, current_val: float, alpha: float, default: float) -> float:
        ema_key = f"{symbol}_ema"
        prev_ema = self.history.get(ema_key, default)
        new_ema = (alpha * current_val) + ((1 - alpha) * prev_ema)
    ...
    def get_osmium_fair(self, state: TradingState) -> float:
        ...
        if best_bid is None or best_ask is None:
            return self.history.get('ASH_COATED_OSMIUM_ema', 10000.0)

        mid = (best_bid + best_ask) / 2.0
        
        # Adaptive Anchor: 100-tick equivalent EMA
        # This protects against the 'peg' moving from 10,000.0
        anchor = self._ema(product, mid, 0.01, 10000.0)
        ...
        mid_pull = max(-1.0, min(1.0, (mid - anchor) * 0.15))
        
        return anchor + tape_adj + mid_pull
    ...
    def run(self, state: TradingState):
        ...
        # ── ASH_COATED_OSMIUM: Adaptive Hybrid Layered MM ──
        product = 'ASH_COATED_OSMIUM'
