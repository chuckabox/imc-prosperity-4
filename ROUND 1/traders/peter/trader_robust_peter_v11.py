"""
trader_robust_peter_v11.py
==========================
V11 — "Aggressive by Default, Safe by Exception"

Goal: match ken_v6c's ~$78k upside while retaining v6d's guardrails.

Evolution from v6d:
  1. PEPPER ENTRY: take `ask <= mid + 1` (was <= mid). "Pay-to-play" —
     trend days price-move faster than our fair, tight-take misses fills.
  2. PEPPER PASSIVE: cap 40 (was 10). In stable windows we want to
     accumulate 4x faster via the resting bid.
  3. PEPPER CAP on STRONG drift: 80 (matches v6c). Regime tiers unchanged.
  4. PEPPER STOP threshold: -12 (was -8). Minor pullbacks no longer trip
     the stop; a real reversal still does.
  5. OSMIUM quote sizes: 25 / 18 (matches v6c). Bigger edge capture.

Retained from v6d (the "safe by exception" half):
  - Smoothed warmup (median of first 30 vs last 30) + cap re-eval (upgrade-only).
  - Magnitude stop + 2-tick hysteresis + recovery on Pepper.
  - Chunked flatten on stop; tentative cap 20 during warmup.
  - Osmium: toxic-flow gate, anchor-sanity downscale, 3-tier skew, clamp ±4.
"""
