...
class Trader:
    """
    Simple trader:
    1) Market making around short-term fair value
    2) Mean-reversion taker on large one-tick shocks
    """

    # Keep scope narrow to stronger names for cleaner behavior.
    SYMBOLS = {"PEBBLES_XL", "MICROCHIP_OVAL"}
    LIMITS = {"PEBBLES_XL": 20, "MICROCHIP_OVAL": 16}
    ...
    def run(self, state: TradingState):
        ...
        for symbol in self.SYMBOLS:
            ...
            lim = self.LIMITS.get(symbol, 12)
            buy_cap = max(0, lim - pos)
            sell_cap = max(0, lim + pos)
            if buy_cap <= 0 and sell_cap <= 0:
                continue

            # Fair value = current mid with small inventory skew.
            fair = mid - self.INV_SKEW * pos

            # Layer 1: passive market making
            ...
            # Layer 2: mean-reversion taker on large shocks with cooldown
