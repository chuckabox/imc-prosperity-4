...
# we_will_win.py — surgical fixes over ken/heaven_we_comingv5a.py.
# Backtest evidence (day 2/3/4) showed prior pair-convergence + shock-fade
# overlays were net-negative: pair-entry crosses two spreads, taker fade hold=1
# leaves no recovery time. Both removed.
#
# Remaining changes vs v5a (each justified by docs OR backtest data):
#   1. Fix _family(): split("_",1)[0] mis-keys SLEEP_POD/OXYGEN_SHAKE/
#      GALAXY_SOUNDS/UV_VISOR. Use prefix table → spread caps actually apply.
#   2. Tier C skip: SNACKPACK (signal_quality<0), GALAXY_SOUNDS non-LEADER
#      (tight_rate ~3%), UV_VISOR_{YELLOW,RED,MAGENTA} (tight_rate <5%),
#      PEBBLES_XL (q=-0.031). Sources: volatility.md, top_symbols csv.
#   3. Empirical bleeder skip: SLEEP_POD_LAMB_WOOL (-28k 3-day),
#      PEBBLES_M (-19k), OXYGEN_SHAKE_MINT (-18k). All 3/3 days negative
#      independent of overlays in run logged 2026-04-30.
#   4. LL skew: short-window momentum × regression β (clamped to ±2),
#      bound by LL_SKEW_CAP=3 (kept). More responsive than v5a's endpoint diff,
#      same magnitude bound so it cannot blow up.
# Untouched: LIMIT=10, MM_CLIP=4, INV_SKEW=0.30, FAMILY_SPREAD_CAPS values,
# passive-MM quote shape, return tuple.
...
# Skip set: doc-tier-C + empirical bleeders.
SKIP_MM = {
    # Tier C — docs say no edge after cost
    ...
    "PEBBLES_XL",
    # Empirical bleeders — 3/3 days negative in latest backtest
    "SLEEP_POD_LAMB_WOOL",
    "PEBBLES_M",
    "OXYGEN_SHAKE_MINT",
}
