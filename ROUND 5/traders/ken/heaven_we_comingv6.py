...
# v6 (new): one-knob iteration from v5a only.
# Change: tighten leader-lag skew cap from 3.0 -> 2.0 for portal stability.
LIMIT = 10
MM_CLIP = 4
INV_SKEW = 0.30
...
# v6: portal-first incremental update over v5a
# Changes:
# 1) Slightly tighter spread caps for noisier families.
# 2) Clip decays when spread is close to family cap.
# Keep core answer/v5a architecture intact.
LIMIT = 10
BASE_MM_CLIP = 4
INV_SKEW = 0.30
LEADER = "GALAXY_SOUNDS_BLACK_HOLES"
...
FAMILY_SPREAD_CAPS = {
    ...
    "PANEL": 10,
    "MICROCHIP": 10,
    "SLEEP_POD": 10,      # tighter than v5a
    "OXYGEN_SHAKE": 11,   # tighter than v5a
    "PEBBLES": 11,        # tighter than v5a
    "UV_VISOR": 11,       # tighter than v5a
    "GALAXY_SOUNDS": 12,
    "SNACKPACK": 11,      # tighter than v5a
}
DEFAULT_SPREAD_CAP = 12
...
class Trader:
    ...
    def _clip_for_spread(self, spread: int, cap: int) -> int:
        # Reduce size when quoting close to the worst acceptable spread.
        if spread >= cap:
            return 2
        if spread == cap - 1:
            return 3
