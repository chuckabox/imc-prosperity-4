"""trader_ken_v63_smile_attack.py

Aggressive smile-residual variant targeting higher upside:
- Broader strike set with 5200/5300 focus
- Softer defense transitions (fewer premature clamps)
- Multi-signal options execution per tick
"""
...
VEV_ALL = [5000, 5100, 5200, 5300, 5400]
VEV_TRADE = [5100, 5200, 5300, 5400]

STRIKE_CAP: Dict[int, int] = {5000: 24, 5100: 30, 5200: 36, 5300: 36, 5400: 28}
VEV_DELTA_APPROX: Dict[int, float] = {5000: 0.82, 5100: 0.70, 5200: 0.56, 5300: 0.44, 5400: 0.31}
...
class Trader:
    ...
    A_VFE_TAKER_MAX = 12
    A_VEV_Z_ENTRY = 1.40
    A_VEV_TAKER = {5100: 6, 5200: 8, 5300: 8, 5400: 5}
    ...
    B_VFE_TAKER_MAX = 7
    B_VEV_Z_ENTRY = 1.80
    B_VEV_TAKER = {5100: 4, 5200: 5, 5300: 5, 5400: 3}

    VEV_SPREAD_MAX = {5000: 8, 5100: 8, 5200: 8, 5300: 8, 5400: 8}
    VEV_MAX_SIGNALS = 2
    NET_DELTA_SOFT = 58.0
    NET_DELTA_HARD = 72.0
    VOL_ALPHA = 0.10
    ...
    def _load(self, state: TradingState):
        ...
        self.h.setdefault("eq_proxy", 0.0)
        self.h.setdefault("eq_peak", 0.0)
        self.h.setdefault("last_mid", {})
        for k in VEV_ALL:
    ...
    @staticmethod
    def _mid(d: Optional[OrderDepth]) -> Optional[float]:
        if d is None:
            return None
        bb = max(d.buy_orders.keys()) if d.buy_orders else None
        ba = min(d.sell_orders.keys()) if d.sell_orders else None
    ...
    def _update_equity_proxy(self, state: TradingState):
        mids: Dict[str, float] = {}
        for sym in [HYDRO, VFE, "VEV_5100", "VEV_5200", "VEV_5300"]:
            m = self._mid(state.order_depths.get(sym))
            if m is not None:
                mids[sym] = m
        last_mid = self.h.get("last_mid", {})
        eq = float(self.h.get("eq_proxy", 0.0))
        for sym, m in mids.items():
            if sym in last_mid:
                pos = int(state.position.get(sym, 0))
                eq += pos * (m - float(last_mid[sym]))
        peak = max(float(self.h.get("eq_peak", 0.0)), eq)
        self.h["eq_proxy"] = eq
        self.h["eq_peak"] = peak
        self.h["last_mid"] = mids
    ...
    def _risk_scale(self, state: TradingState, dd: float) -> float:
        nd = abs(self._net_delta(state))
        base = 1.0
        if nd >= self.NET_DELTA_HARD:
            base = 0.35
        elif nd >= self.NET_DELTA_SOFT:
            base = 0.65
        if dd >= self.DD_CRISIS:
            return base * 0.30
        if dd >= self.DD_HARD:
            return base * 0.50
        if dd >= self.DD_SOFT:
    ...
    def _update_policy(self, vfe_mid: Optional[float], hydro_spread: Optional[int], dd: float):
        jump = 0.0
        ...
        if on:
            if c < self.COST_OFF and dd < self.DD_SOFT:
                on = False
        else:
            if c > self.COST_ON or dd >= self.DD_HARD:
                on = True
    ...
    def _hydro(self, state: TradingState, scale: float, defense: bool, maker_only: bool):
        ...
        out: List[Order] = []
        if not maker_only and ba - bb <= 13:
    ...
    def _vfe(self, state: TradingState, scale: float, defense: bool, maker_only: bool):
    ...
    def _vev_smile(self, state: TradingState, vfe_mid: float, scale: float, defense: bool, maker_only: bool):
        if maker_only:
            return []
        if defense:
        ...
        for k in VEV_TRADE:
            ...
            z = resid / (nv ** 0.5)
            # Slight score boost for historically strongest strikes.
            boost = 0.15 if k in (5200, 5300) else 0.0
            cands.append((abs(z) + boost, z, k, d, bb, ba, fair))
        ...
        out = []
        used = 0
        for _, z, k, d, bb, ba, fair in cands:
            if used >= self.VEV_MAX_SIGNALS:
                break
            if abs(z) < z_entry:
                continue
    ...
    def run(self, state: TradingState):
        ...
        dd = self._update_equity_proxy(state)

        vfe_mid = self._mid(state.order_depths.get(VFE))
        h_spread = None
        ...
        defense = self._update_policy(vfe_mid, h_spread, dd)
        scale = self._risk_scale(state, dd)
        maker_only = dd >= self.DD_CRISIS

        for o in self._hydro(state, scale, defense, maker_only):
            res.setdefault(o.symbol, []).append(o)
        vfe_orders, vfe_mid2 = self._vfe(state, scale, defense, maker_only)
        for o in vfe_orders:
            res.setdefault(o.symbol, []).append(o)

        vref = vfe_mid2 if vfe_mid2 is not None else vfe_mid
        if vref is not None:
            for o in self._vev_smile(state, vref, scale, defense, maker_only):
                res.setdefault(o.symbol, []).append(o)
