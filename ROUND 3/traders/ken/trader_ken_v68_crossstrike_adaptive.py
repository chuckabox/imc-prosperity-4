"""trader_ken_v68_crossstrike_adaptive.py
...
- Adds adaptive pair sizing and persistence gating.
"""
...
class Trader:
    ...
    VEV_REL_Z_ENTRY = 1.10
    VEV_REL_Z_STRONG = 1.45
    VEV_MAX_FALLBACK_STREAK = 8
    ...
    def _load_state(self, state: TradingState) -> None:
        ...
        self.history.setdefault("vev_pair_ema", 0.0)
        self.history.setdefault("vev_fallback_streak", 0)
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool) -> List[Order]:
        ...
        pair_score = 0.0
        if rich and cheap:
            pair_score = 0.5 * (rich[0][3] - cheap[0][3])
        pair_ema = float(self.history.get("vev_pair_ema", 0.0))
        pair_ema = 0.85 * pair_ema + 0.15 * pair_score
        self.history["vev_pair_ema"] = pair_ema
        ...
        if rich and cheap:
            ...
            if sell_k != buy_k:
                ...
                q_sell_cap = min(sell_mx, sell_lim + sell_pos, sell_d.buy_orders.get(sell_bb, 0))
                q_buy_cap = min(buy_mx, buy_lim - buy_pos, -buy_d.sell_orders.get(buy_ba, 0))

                ds = max(0.05, VEV_DELTA_APPROX[sell_k])
                db = max(0.05, VEV_DELTA_APPROX[buy_k])
                # Approx delta-neutral pair sizing: q_buy * db ~= q_sell * ds
                q_sell_from_buy = int(q_buy_cap * db / ds)
                q_buy_from_sell = int(q_sell_cap * ds / db)
                q_sell = min(q_sell_cap, max(1, q_sell_from_buy))
                q_buy = min(q_buy_cap, max(1, q_buy_from_sell))

                # Regime gating: require stronger score or persistent dislocation.
                strong_enough = pair_score >= self.VEV_REL_Z_STRONG
                persistent_enough = pair_ema >= self.VEV_REL_Z_ENTRY
                if q_sell > 0 and q_buy > 0 and (strong_enough or persistent_enough):
                    orders.append(Order(sell_sym, sell_bb, -q_sell))
                    orders.append(Order(buy_sym, buy_ba, q_buy))
                    self.history["vev_fallback_streak"] = 0
                    return orders

        # Fallback single-leg strongest absolute signal.
        self.history["vev_fallback_streak"] = int(self.history.get("vev_fallback_streak", 0)) + 1
        if int(self.history["vev_fallback_streak"]) > self.VEV_MAX_FALLBACK_STREAK:
            return []
