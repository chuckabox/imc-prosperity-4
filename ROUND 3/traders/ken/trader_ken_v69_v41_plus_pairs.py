"""trader_ken_v41_standalone.py

Standalone portal-safe v41 variant.
- Keeps v35 hydrogel/vfe backbone.
- Options concentrated to strongest pair (5000/5100) with sparse taker logic.
- Adds tiny high-threshold 5200/5300 relative-value pair addon.
"""
...
class Trader:
    ...
    PREM_VAR_ALPHA = 0.08
    VEV_Z_ENTRY = 1.65
    VEV_SPREAD_MAX_BY_STRIKE: Dict[int, int] = {5000: 8, 5100: 6}
    VEV_TAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 9, 5100: 6}
    VEV_PAIR_Z_ENTRY = 1.9
    VEV_PAIR_MAX_Q = 2
    ...
    def _load_state(self, state: TradingState) -> None:
        ...
        self.history.setdefault("pair_rel_mu", 0.0)
        self.history.setdefault("pair_rel_var", 9.0)
    ...
    def _vev_pair_addon(self, state: TradingState, vfe_mid: float) -> List[Order]:
        d52 = state.order_depths.get("VEV_5200")
        d53 = state.order_depths.get("VEV_5300")
        if d52 is None or d53 is None:
            return []
        bb52, ba52 = self._top(d52)
        bb53, ba53 = self._top(d53)
        if bb52 is None or ba52 is None or bb53 is None or ba53 is None:
            return []
        if (ba52 - bb52) > 8 or (ba53 - bb53) > 8:
            return []

        m52 = (bb52 + ba52) / 2.0
        m53 = (bb53 + ba53) / 2.0
        ext52 = m52 - max(vfe_mid - 5200, 0.0)
        ext53 = m53 - max(vfe_mid - 5300, 0.0)
        rel = ext52 - ext53

        mu = float(self.history.get("pair_rel_mu", 0.0))
        var = float(self.history.get("pair_rel_var", 9.0))
        mu = 0.94 * mu + 0.06 * rel
        dev = rel - mu
        var = 0.92 * var + 0.08 * (dev * dev)
        var = max(1.0, var)
        self.history["pair_rel_mu"] = mu
        self.history["pair_rel_var"] = var
        z = dev / (var ** 0.5)
        if abs(z) < self.VEV_PAIR_Z_ENTRY:
            return []

        pos52 = int(state.position.get("VEV_5200", 0))
        pos53 = int(state.position.get("VEV_5300", 0))
        lim52 = 30
        lim53 = 30
        q = self.VEV_PAIR_MAX_Q
        out: List[Order] = []

        # rel too high => 5200 rich vs 5300: sell 5200, buy 5300 (delta-balanced-ish)
        if z > 0:
            q_sell52 = min(q, lim52 + pos52, d52.buy_orders.get(bb52, 0))
            q_buy53 = min(max(1, int(round(q_sell52 * 0.57 / 0.44))), lim53 - pos53, -d53.sell_orders.get(ba53, 0))
            q_use = min(q_sell52, max(1, int(round(q_buy53 * 0.44 / 0.57))))
            if q_use > 0 and q_buy53 > 0:
                out.append(Order("VEV_5200", bb52, -q_use))
                out.append(Order("VEV_5300", ba53, q_buy53))
        else:
            q_buy52 = min(q, lim52 - pos52, -d52.sell_orders.get(ba52, 0))
            q_sell53 = min(max(1, int(round(q_buy52 * 0.57 / 0.44))), lim53 + pos53, d53.buy_orders.get(bb53, 0))
            q_use = min(q_buy52, max(1, int(round(q_sell53 * 0.44 / 0.57))))
            if q_use > 0 and q_sell53 > 0:
                out.append(Order("VEV_5200", ba52, q_use))
                out.append(Order("VEV_5300", bb53, -q_sell53))
        return out
    ...
    def run(self, state: TradingState):
        ...
        if self.ENABLE_VEV and vfe_mid is not None:
            for o in self._vev_logic(state, vfe_mid, scale, risk_off):
                result.setdefault(o.symbol, []).append(o)
            for o in self._vev_pair_addon(state, vfe_mid):
                result.setdefault(o.symbol, []).append(o)
