"""trader_ken_v20.py — Inventory-Controlled Engine

Root cause of v18/v19 drawdowns identified: VEV never fills (0 trades),
so the VEV cap in v19 had zero effect. The drawdowns are entirely from
HYDROGEL inventory risk.

Problem: after a taker fills 20 units, the guarded maker posts the REMAINING
60 units as one order. When that fills, we instantly hold 80 units. A 20-tick
adverse move costs 80 × 20 = 1,600 — matching the observed -1,500 valleys.

Fix: cap HYDROGEL maker orders at HP_MAKER_SIZE = 15 per side. Inventory
builds gradually (max ~35/tick), giving HYDROGEL time to mean-revert before
we're at max exposure. Position limit stays at 80 — we still get the full
benefit, just without the sudden 60-unit fill spike.
"""
...
class Trader:
    ...
    HP_MAKER_EDGE = 2.0
    HP_TAKER_MAX = 20
    HP_MAKER_SIZE = 15      # FIX: cap maker per side — prevents instant 60-unit bulk fill
    ...
    def _hydrogel_logic(self, state: TradingState) -> List[Order]:
        ...
        # Maker: capped at HP_MAKER_SIZE per side to prevent sudden bulk fills
        orders.extend(self._guarded_maker(HYDROGEL, depth, pos, fair, lim, self.HP_MAKER_EDGE, max_qty=self.HP_MAKER_SIZE))
        return orders
