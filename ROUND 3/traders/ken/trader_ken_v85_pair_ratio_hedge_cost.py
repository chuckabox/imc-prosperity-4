"""trader_ken_v85_pair_ratio_hedge_cost.py

Standalone v85:
- Builds on v84 pair-RV concept
- Uses delta-aware pair ratio sizing (cheap-leg qty vs rich-leg qty)
- Adds hedge-pressure and VFE-spread-aware throttling for new VEV risk
"""
...
class Trader:
    ...
    def _vfe(self, state: TradingState, target_pos: int) -> List[Order]:
        ...
        allow_alpha_take = spread <= self.VFE_TAKER_SPREAD_MAX
        if allow_alpha_take and abs(target_pos - pos) <= self.VFE_HEDGE_AGGRO_BAND:
            rem = taker_max
    ...
    def _vev(self, state: TradingState) -> List[Order]:
        ...
        if vfe_od and vfe_od.buy_orders and vfe_od.sell_orders:
            vfe_spread = min(vfe_od.sell_orders) - max(vfe_od.buy_orders)
        pressure_scale = 1.0
        if hedge_pressure >= 0.70:
            pressure_scale *= 0.7
            entry += 0.3
        if vfe_spread > self.VFE_TAKER_SPREAD_MAX:
            pressure_scale *= 0.75
            entry += 0.2
        ...
        if low_bucket and high_bucket:
            ...
            if cheap_val <= -entry and rich_val >= entry:
                ...
                if cba is not None and rbb is not None:
                    ...
                    d_buy = max(0.05, self.DELTA_APPROX[cheap_k])
                    d_sell = max(0.05, self.DELTA_APPROX[rich_k])
                    ratio = max(0.5, min(1.8, d_buy / d_sell))
                    q_buy = max(1, int(self.VEV_PAIR_BASE_QTY * pressure_scale))
                    q_sell = max(1, int(round(q_buy * ratio)))
                    q_buy = min(q_buy, buy_room, budget)
                    q_sell = min(q_sell, sell_room, budget)
                    if q_buy > 0 and q_sell > 0:
                        orders.append(Order(cheap_sym, cba, q_buy))
                        orders.append(Order(rich_sym, rbb, -q_sell))
                        abs_pos += q_buy + q_sell
