"""trader_ken_v23.py — v22 + trailing drawdown circuit breaker.

Purpose:
- Keep v22's upside behavior.
- Cut "peak-to-trough giveback" by switching to temporary protect mode when
  proxy mark-to-market drawdown exceeds a threshold.
"""
...
class Trader:
    ...
    def _risk_state(self, state: TradingState, vfe_mid: Optional[float], protect_mode: bool) -> Tuple[float, bool]:
        ...
        risk_off = protect_mode or hp_pos >= self.RISK_HP_POS_TRIGGER or net_delta >= self.RISK_NET_DELTA_TRIGGER or score >= 0.95
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool, protect_mode: bool) -> List[Order]:
        ...
        edge = self.HP_MAKER_EDGE + (1.2 if risk_off else 0.0)
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool, protect_mode: bool) -> List[Order]:
        ...
        for k in VEV_STRIKES:
            ...
            lim = max(8, int(STRIKE_CAP[k] * (0.65 if protect_mode else (0.75 if risk_off else 1.0))))
            edge = self.VEV_MAKER_EDGE + (1.0 if risk_off else 0.0)
