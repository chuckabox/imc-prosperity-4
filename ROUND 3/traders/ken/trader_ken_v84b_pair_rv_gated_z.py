"""trader_ken_v84b_pair_rv_gated_z.py

Standalone v84b:
- Starts from v84 pair-RV baseline
- Adds conservative, gated z-score overlays to HYDROGEL only when
  market conditions are favorable (tight spread + balanced top-of-book).
"""
...
class Trader:
    ...
    # HYDROGEL gated z-score overlay
    HP_Z_VAR_ALPHA = 0.08
    HP_Z_ENTER = 1.8
    HP_Z_GATE_SPREAD_MAX = 15
    HP_Z_OBI_MAX = 0.35
    HP_Z_TAKER_MAX = 14
    ...
    def _load(self, state: TradingState) -> None:
        ...
        self.history.setdefault("hp_z_var", 9.0)
    ...
    def _hp(self, state: TradingState) -> List[Order]:
        ...
        dev = m - fair
        zvar = (1 - self.HP_Z_VAR_ALPHA) * float(self.history["hp_z_var"]) + self.HP_Z_VAR_ALPHA * (dev * dev)
        zvar = max(1.0, zvar)
        self.history["hp_z_var"] = zvar
        z = dev / math.sqrt(zvar)
        ...
        # Gated z-score overlay (conservative add-on)
        spread = None if (bb is None or ba is None) else (ba - bb)
        gate_ok = False
        if spread is not None and spread <= self.HP_Z_GATE_SPREAD_MAX and (bv + av) > 0:
            obi = (bv - av) / float(bv + av)
            if abs(obi) <= self.HP_Z_OBI_MAX:
                gate_ok = True
        if gate_ok:
            if z <= -self.HP_Z_ENTER and ba is not None:
                qty = min(self.HP_Z_TAKER_MAX, lim - pos, -od.sell_orders[ba])
                if qty > 0:
                    orders.append(Order(HYDROGEL, ba, qty))
                    pos += qty
            elif z >= self.HP_Z_ENTER and bb is not None:
                qty = min(self.HP_Z_TAKER_MAX, lim + pos, od.buy_orders[bb])
                if qty > 0:
                    orders.append(Order(HYDROGEL, bb, -qty))
                    pos -= qty

        spread_term = 1 + int(vol * 2)
        skew = int(round(3 * (pos / lim)))
        bid_px = int(round(fair - spread_term - skew))
        ask_px = int(round(fair + spread_term - skew))
        if bb is not None:
