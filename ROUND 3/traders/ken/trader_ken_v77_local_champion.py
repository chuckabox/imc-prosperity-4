...
class Trader:
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool) -> List[Order]:
        ...
        rich = [x for x in raw if x[3] >= self.VEV_REL_Z_ENTRY]
        cheap = [x for x in raw if x[3] <= -self.VEV_REL_Z_ENTRY]
        ...
        if rich and cheap:
            ...
            if sell_k != buy_k:
                ...
                local_scale = scale * (self.VEV_STRONG_PAIR_SIZE_MULT if strong_pair else 1.0)
                sell_mx = max(1, int(self.VEV_TAKER_MAX_BY_STRIKE[sell_k] * local_scale))
                buy_mx = max(1, int(self.VEV_TAKER_MAX_BY_STRIKE[buy_k] * local_scale))
