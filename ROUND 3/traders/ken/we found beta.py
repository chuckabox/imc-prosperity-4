...
Meta-controller variant of "we found alpha":
- Regime modes: aggressive, defensive, hydrogel_only
- Controller uses live stress signals (VFE spread, hedge pressure, VEV inventory)
- Same core modules, but risk budget and entry gates are mode-dependent
"""
...
class Trader:
    ...
    CTRL_VFE_SPREAD_DEF = 6
    CTRL_VFE_SPREAD_HP_ONLY = 9
    CTRL_HEDGE_PRESSURE_DEF = 0.78
    CTRL_HEDGE_PRESSURE_HP_ONLY = 0.92
    CTRL_VEV_INV_DEF = 0.82
    CTRL_MIN_DEFENSE_TICKS = 30_000
    ...
    def _load(self, state: TradingState) -> None:
        ...
        self.history.setdefault("mode", "aggressive")
        self.history.setdefault("mode_until", -1)
    ...
    def _mode(self, state: TradingState) -> str:
        ts = int(state.timestamp)
        current = str(self.history.get("mode", "aggressive"))
        sticky_until = int(self.history.get("mode_until", -1))
        if ts <= sticky_until:
            return current
        ...
        if VFE in state.order_depths:
            ...
            if bb is not None and ba is not None:
                vfe_spread = ba - bb
        hedge_pressure = abs(state.position.get(VFE, 0)) / float(self.LIMITS[VFE])
        vev_abs = sum(abs(state.position.get(f"VEV_{k}", 0)) for k in self.VEV_FIT_STRIKES)
        vev_pressure = vev_abs / float(max(1, self.VEV_GLOBAL_ABS_CAP))

        mode = "aggressive"
        if (
            vfe_spread >= self.CTRL_VFE_SPREAD_HP_ONLY
            or hedge_pressure >= self.CTRL_HEDGE_PRESSURE_HP_ONLY
        ):
            mode = "hydrogel_only"
        elif (
            vfe_spread >= self.CTRL_VFE_SPREAD_DEF
            or hedge_pressure >= self.CTRL_HEDGE_PRESSURE_DEF
            or vev_pressure >= self.CTRL_VEV_INV_DEF
        ):
            mode = "defensive"

        if mode != current:
            self.history["mode"] = mode
            self.history["mode_until"] = ts + self.CTRL_MIN_DEFENSE_TICKS
        return mode
    ...
    def _vfe(self, state: TradingState, target_pos: int, mode: str) -> List[Order]:
        ...
        if abs(residual) >= self.VFE_HEDGE_BAND:
            ...
            if residual > 0 and pos < lim:
                ...
                if hq > 0:
                    ...
                    pos -= hq
        # alpha-taker only in aggressive mode
        if mode == "aggressive":
        ...
        if qbid >= qask:
            qbid = qask - 1
        maker_max = max(20, int(128 * local_scale))
        if lim - pos > 0:
    ...
    def _vev(self, state: TradingState, mode: str) -> List[Order]:
        if mode == "hydrogel_only":
            return []
        if VFE not in state.order_depths:
            return []
        ...
        if low_bucket and high_bucket:
            ...
            if cheap_val <= -entry and rich_val >= entry:
                ...
                if cba is not None and rbb is not None:
                    ...
                    base_qty = self.VEV_PAIR_MAX_QTY if mode == "aggressive" else max(4, self.VEV_PAIR_MAX_QTY // 2)
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        vev_orders = self._vev(state, mode)
        for o in vev_orders:
            result.setdefault(o.symbol, []).append(o)

        target_vfe = self._target_vfe_from_delta(state)
        vfe_orders = self._vfe(state, target_vfe, mode)
        if vfe_orders:
