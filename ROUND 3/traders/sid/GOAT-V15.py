"""GOAT V15 - V14 + Adaptive HYDROGEL Anchor

V14 logs showed HYDROGEL bleeding ~$2k from a fixed anchor of 9991 while
market mid lived at 10020-10030. Fair was pulled 9 ticks below true mid,
so we got lifted on shorts at 10019 when market would pay 10027.

V15 changes:
1. HP_ANCHOR is now a slow-EMA "drift anchor" instead of a fixed constant.
   - Initialized from first observed mid, then updated with alpha=0.003.
   - Keeps the mean-reversion blend logic (0.8 fast + 0.2 slow_anchor)
     but adapts to regime changes within hours instead of being stuck.
2. HP take/quote logic gets a sanity clamp: never quote our ask BELOW
   (mid - 1) and never quote our bid ABOVE (mid + 1). Prevents the
   "give away free $$" pattern when fair is mispriced.
3. HP_BLEND raised slightly (0.8 -> 0.85) since the slow anchor is now
   trustworthy — gives mean-reversion more weight.

Everything else from V14 is unchanged:
- Ken's vfe gold Greeks-aware delta hedging
- Smile-fit pair trading (5000/5100 vs 5200/5300)
- Smile-based passive MM on 5200/5300/5400/5500
- Phase2 theta-aware exits
- Deep-ITM intrinsic capture on 4000/4500
- VFE micro-structure fair + speed limiting
"""
...
class Trader:
    ...
    # HYDROGEL — adaptive anchor
    HP_BLEND = 0.85                  # Heavier on fast EWMA (slow anchor is now trustworthy)
    HP_EWMA_ALPHA = 0.20             # Fast EMA for fair tracking
    HP_ANCHOR_ALPHA = 0.003          # VERY slow anchor; adapts over many ticks
    HP_VOL_ALPHA = 0.10
    HP_TAKE_EDGE = 1
    HP_QUOTE_SIZE = 85
    ...
    # VEV cross-strike RV (Ken's exact)
    VEV_TTE_START = 8.0
    VEV_DAY_INIT = 2
    ...
    def _load(self, state: TradingState) -> None:
        ...
        self.history.setdefault("hp_anchor", None)        # NEW: adaptive anchor
    ...
    def _hp(self, state: TradingState) -> List[Order]:
        ...
        # ADAPTIVE slow anchor — replaces fixed 9991
        anchor_prev = self.history.get("hp_anchor")
        if anchor_prev is None:
            anchor = m
        else:
            anchor = self.HP_ANCHOR_ALPHA * m + (1 - self.HP_ANCHOR_ALPHA) * anchor_prev
        self.history["hp_anchor"] = anchor
        ...
        fair = self.HP_BLEND * ewma + (1 - self.HP_BLEND) * anchor
        ...
        # Quote with safety clamp around mid
        spread = 1 + int(vol * 2)
        skew = int(round(3 * (pos / lim)))
        bid_px = int(round(fair - spread - skew))
        ask_px = int(round(fair + spread - skew))

        # SAFETY CLAMP: never give away edge by quoting through mid
        # Our bid must NOT exceed mid (don't pay more than fair value)
        # Our ask must NOT be below mid (don't sell below fair value)
        mid_int = int(round(m))
        bid_px = min(bid_px, mid_int)
        ask_px = max(ask_px, mid_int)
    ...
    def _vev_itm(self, state: TradingState, S: Optional[float]) -> List[Order]:
        ...
        for k in self.ITM_STRIKES:
            ...
            if ba <= intrinsic and pos < self.ITM_POS_CAP:
                qty = min(-od.sell_orders[ba], self.ITM_POS_CAP - pos, lim - pos)
                if qty > 0:
                    orders.append(Order(sym, ba, qty))
                    pos += qty
            if bb >= intrinsic + self.ITM_EDGE + 1 and pos > -self.ITM_POS_CAP:
                qty = min(od.buy_orders[bb], self.ITM_POS_CAP + pos, lim + pos)
                if qty > 0:
                    orders.append(Order(sym, bb, -qty))
                    pos -= qty

            bid_px = int(math.floor(intrinsic))
            ask_px = int(math.ceil(intrinsic + self.ITM_EDGE + 1))
            if bid_px >= ba:
                bid_px = ba - 1
            if ask_px <= bb:
                ask_px = bb + 1
