"""GOAT V14 - Ken's vfe gold + Deep-ITM intrinsic capture

V13 broke the pair-trade pipeline (NameError on cheap_od/rich_od) and
drifted from Ken's proven parameters. V14 reverts to Ken's exact code
with only ONE additive enhancement:

  + Deep-ITM intrinsic MM on VEV_4000 and VEV_4500
    These deep-ITM strikes don't fit the smile (they trade ~ S - K).
    trader100.py captured edge here via wide-spread inside MM.
    Ken's strategy doesn't touch them, leaving free $ on the table.

Everything else is Ken's "we found vfe gold.py" verbatim:
