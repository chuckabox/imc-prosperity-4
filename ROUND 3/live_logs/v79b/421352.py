...
Standalone full-send variant for portal submission.
- Peter-style aggression: deeper taker sweep + tighter maker quotes.
- Cross-strike VEV relative-value + passive quotes + structural long-vol bias.
- Keeps VFE hedge loop so option exposure does not run away.
"""
...
class Trader:
    ...
    VEV_TAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 9, 5100: 11, 5200: 16, 5300: 16, 5400: 9}
    VEV_REL_Z_ENTRY = 0.62
    VEV_REL_Z_BOOST = 1.35
    VEV_STRONG_SPREAD_MAX = 3
    VEV_STRONG_PAIR_SIZE_MULT = 1.72
    VEV_MAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 7, 5100: 8, 5200: 12, 5300: 12, 5400: 7}
    VEV_MAKER_EDGE = 0.9
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        taker_max = max(16, int(self.HP_TAKER_MAX * local_scale))
        maker_max = max(46, int(156 * local_scale))
