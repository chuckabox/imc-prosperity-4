"""trader_ken_v49_live_exec.py

Live-execution-focused variant from portal gap analysis:
- Strong maker-first Hydrogel profile
- Very strict Hydrogel taker controls (spread gate + cooldown + deeper edge)
- Sparse options execution retained
"""
...
PREM_INIT: Dict[int, float] = {5000: 5.81, 5100: 19.09, 5200: 48.85}
PREM_BOUNDS: Dict[int, Tuple[float, float]] = {5000: (2.0, 14.0), 5100: (10.0, 30.0), 5200: (30.0, 72.0)}
STRIKE_CAP: Dict[int, int] = {5000: 34, 5100: 28, 5200: 22}
VEV_DELTA_APPROX: Dict[int, float] = {5000: 0.82, 5100: 0.70, 5200: 0.57}
...
class Trader:
    ...
    # Hydro taker now intentionally strict.
    HP_TAKER_EDGE = 5.5
    HP_TAKER_MAX = 6
    HP_TAKER_SPREAD_MAX = 10
    HP_TAKER_COOLDOWN_TS = 1200
    ...
    PREM_VAR_ALPHA = 0.08
    VEV_Z_ENTRY = 1.75
    VEV_SPREAD_MAX_BY_STRIKE: Dict[int, int] = {5000: 8, 5100: 6, 5200: 4}
    ...
    def _load_state(self, state: TradingState) -> None:
        ...
        self.history.setdefault("hp_taker_until", -1)
    ...
    def _guarded_maker(
        self, symbol: str, depth: OrderDepth, pos: int, fair: float, limit: int, edge: float, max_qty: Optional[int] = None
    ) -> List[Order]:
        orders = []
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        now = int(state.timestamp)
        spread = ba - bb
        taker_max = max(1, int(self.HP_TAKER_MAX * local_scale))
        maker_max = max(8, int(self.HP_MAKER_MAX * local_scale))

        # Taker only if edge is deep, spread is not too wide, and we're not on cooldown.
        taker_ok = (
            (not risk_off)
            and (not speed_limited)
            and now >= int(self.history.get("hp_taker_until", -1))
            and spread <= self.HP_TAKER_SPREAD_MAX
        )

        # Also bias taker toward inventory reduction to avoid repeated adverse crossing.
        if taker_ok and ba <= fair - self.HP_TAKER_EDGE and pos < lim and pos <= 30:
            sz = min(taker_max, lim - pos, -depth.sell_orders[ba])
            if sz > 0:
                orders.append(Order(HYDROGEL, ba, sz))
                pos += sz
                self.history["hp_taker_until"] = now + self.HP_TAKER_COOLDOWN_TS
        if taker_ok and bb >= fair + self.HP_TAKER_EDGE and pos > -lim and pos >= -30:
    ...
    def _vfe_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        maker_max = max(4, int(16 * local_scale))
