...
class Trader:
    ...
    # Startup floor protection
    STARTUP_TICKS = 700
    STARTUP_LIMIT_SCALE = 0.60
    STARTUP_MAKER_SCALE = 0.55
    STARTUP_ANCHOR_WEIGHT = 0.65
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool, startup_mode: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        if startup_mode:
            fair = (1.0 - self.STARTUP_ANCHOR_WEIGHT) * ewma + self.STARTUP_ANCHOR_WEIGHT * self.HP_ANCHOR
        else:
        ...
        if startup_mode:
            lim = max(8, int(self.HP_LIMIT * self.STARTUP_LIMIT_SCALE))
            maker_max = max(4, int(maker_max * self.STARTUP_MAKER_SCALE))
        taker_enabled = (not risk_off) and (not startup_mode)
        ...
        maker_edge = self.HP_MAKER_EDGE + (1.0 if risk_off else 0.0) + (0.7 if startup_mode else 0.0)
        orders.extend(self._guarded_maker(HYDROGEL, depth, pos, fair, lim, maker_edge, max_qty=maker_max))
        return orders, mid

    def _vfe_logic(self, state: TradingState, scale: float, risk_off: bool, startup_mode: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        if startup_mode:
            lim = max(8, int(self.VFE_LIMIT * self.STARTUP_LIMIT_SCALE))
            maker_max = max(3, int(maker_max * self.STARTUP_MAKER_SCALE))

        # In risk-off, disable aggressive taker to reduce churn.
        if (not risk_off) and (not startup_mode) and ba <= fair - self.VFE_TAKER_EDGE and pos < lim:
            sz = min(taker_max, lim - pos, -depth.sell_orders[ba])
            if sz > 0:
                orders.append(Order(VFE, ba, sz))
                pos += sz
        if (not risk_off) and (not startup_mode) and bb >= fair + self.VFE_TAKER_EDGE and pos > -lim:
            sz = min(taker_max, lim + pos, depth.buy_orders[bb])
            if sz > 0:
                orders.append(Order(VFE, bb, -sz))
                pos -= sz

        maker_edge = self.VFE_MAKER_EDGE + (1.0 if risk_off else 0.0) + (0.5 if startup_mode else 0.0)
        orders.extend(self._guarded_maker(VFE, depth, pos, fair, lim, maker_edge, max_qty=maker_max))
        return orders, mid

    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool, startup_mode: bool) -> List[Order]:
        orders: List[Order] = []
        maker_max = max(2, int(10 * scale))
        if startup_mode:
            maker_max = max(2, int(maker_max * self.STARTUP_MAKER_SCALE))
        ...
        for strike in VEV_STRIKES:
            ...
            if startup_mode:
                lim = max(6, int(lim * self.STARTUP_LIMIT_SCALE))

            edge = self.VEV_MAKER_EDGE + (0.8 if risk_off else 0.0) + (0.6 if startup_mode else 0.0)
            orders.extend(
    ...
    def run(self, state: TradingState):
        ...
        self.history["ticks"] = int(self.history.get("ticks", 0)) + 1
        ...
        startup_mode = int(self.history.get("ticks", 0)) <= self.STARTUP_TICKS

        if self.ENABLE_HYDROGEL:
            hp_orders, hp_mid_exec = self._hydrogel_logic(state, scale, risk_off, startup_mode)
            for o in hp_orders:
                result.setdefault(o.symbol, []).append(o)
            if hp_mid_exec is not None:
                self.history["last_hp_mid"] = hp_mid_exec

        vfe_orders, vfe_mid = self._vfe_logic(state, scale, risk_off, startup_mode)
        if self.ENABLE_VFE:
            for o in vfe_orders:
                result.setdefault(o.symbol, []).append(o)
        ...
        if self.ENABLE_VEV and vfe_mid is not None:
            for o in self._vev_logic(state, vfe_mid, scale, risk_off, startup_mode):
                result.setdefault(o.symbol, []).append(o)
