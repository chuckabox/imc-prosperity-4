"""trader_ken_v79_portal_recovery.py

Portal-oriented upgrade from v78:
- Higher throughput on HYDROGEL/VFE.
- VEV now has both taker RV and passive maker quotes (previously mostly taker-only).
- Structural long-vol accumulation on 5200/5300 when cheap, with VFE delta hedge.
"""
...
# Portal tends to reward throughput; run wider limits with risk scaling.
...
class Trader:
    ...
    VEV_TAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 7, 5100: 8, 5200: 11, 5300: 11, 5400: 7}
    VEV_REL_Z_ENTRY = 0.80
    VEV_REL_Z_BOOST = 1.55
    VEV_STRONG_SPREAD_MAX = 3
    VEV_STRONG_PAIR_SIZE_MULT = 1.50
    VEV_MAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 5, 5100: 6, 5200: 8, 5300: 8, 5400: 5}
    VEV_MAKER_EDGE = 1.2
    VEV_STRUCT_LONG_Z = -0.55
    VEV_STRUCT_LONG_SIZE = 4

    # Risk governor
    RISK_NET_DELTA_TRIGGER = 95.0
    RISK_HP_POS_TRIGGER = 160
    RISK_ADVERSE_MOVE = 2.6
    RISK_MIN_SCALE = 0.30
    RISK_VEV_GUARD_TS = 0
    RISK_VEV_GUARD_DECAY = 0.978
    RISK_VEV_GUARD_SCALE_CUT = 0.22
    RISK_VEV_GUARD_Z_BUMP = 0.10
    RISK_VEV_GUARD_REL_BUMP = 0.10

    # Soft safety
    OPEN_PHASE_TS = 120_000
    HP_SPEED_TRIGGER = 40
    VFE_SPEED_TRIGGER = 34
    SPEED_COOLDOWN_TS = 40_000
    OPEN_SCALE_MULT = 0.90
    SPEED_SCALE_MULT = 0.78
    ...
    def _risk_state(
        ...
        if vfe_mid is not None and last_vfe is not None:
            ...
            if (signed > 0 and dv < -self.RISK_ADVERSE_MOVE) or (
                ...
                self.history["vev_guard_level"] = min(
                    1.0,
                    float(self.history.get("vev_guard_level", 0.0)) + 0.30,
                )
            else:
    ...
    def _speed_limited(
        self, state: TradingState, symbol: str, trigger: int, cd_key: str, last_pos_key: str
    ) -> bool:
        now = int(state.timestamp)
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        taker_max = max(8, int(self.HP_TAKER_MAX * local_scale))
        maker_max = max(20, int(84 * local_scale))
    ...
    def _vfe_logic(
        self, state: TradingState, scale: float, risk_off: bool, target_pos: Optional[int] = None
        ...
        # Hedge first to keep options PnL clean.
        if target_pos is not None:
            residual = target_pos - pos
            if abs(residual) >= self.VFE_HEDGE_BAND:
                hmx = max(5, int(self.VFE_HEDGE_MAX * local_scale))
        ...
        taker_max = max(5, int(self.VFE_TAKER_MAX * local_scale))
        maker_max = max(14, int(64 * local_scale))

        # Suppress directional taking while far from hedge target.
        ...
        if allow_alpha_take and ba <= fair - self.VFE_TAKER_EDGE and pos < lim:
            sz = min(taker_max, lim - pos, -depth.sell_orders[ba])
            if sz > 0:
                orders.append(Order(VFE, ba, sz))
                pos += sz
        if allow_alpha_take and bb >= fair + self.VFE_TAKER_EDGE and pos > -lim:
            sz = min(taker_max, lim + pos, depth.buy_orders[bb])
            if sz > 0:
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool) -> List[Order]:
        ...
        local_scale = max(0.24, local_scale)
        ...
        per_strike: Dict[int, Tuple[int, int, float, float]] = {}
        for strike in VEV_STRIKES:
            ...
            per_strike[strike] = (bb, ba, z, fair)
        ...
        # Passive maker on each strike + structural long-vol tilt on core strikes.
        for k, (kbb, kba, kz, kfair) in per_strike.items():
            ksym = f"VEV_{k}"
            ...
            # Base symmetric maker.
            orders.extend(self._guarded_maker(ksym, kdepth, kpos, kfair, klim, self.VEV_MAKER_EDGE, max_qty=kmx))

            # Persistent bias: add one extra bid on core strikes when cheap.
            if k in CORE_LONG_STRIKES and kz <= self.VEV_STRUCT_LONG_Z and kpos < klim:
    ...
    def run(self, state: TradingState):
        ...
        # Stage 1: VEV orders to define target hedge.
        if self.ENABLE_VEV and vfe_mid_for_risk is not None:
            for o in self._vev_logic(state, vfe_mid_for_risk, scale, risk_off):
                result.setdefault(o.symbol, []).append(o)

        # Target VFE from current options inventory.
        target_vfe = int(round(-sum(
            state.position.get(f"VEV_{k}", 0) * VEV_DELTA_APPROX[k] for k in VEV_STRIKES
        )))
