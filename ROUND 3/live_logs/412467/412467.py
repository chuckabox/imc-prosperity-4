...
Iteration target: beat v30 while keeping stable core.

Design:
- Keep v30 hydrogel/vfe/risk backbone (strong baseline stability).
- Replace broad VEV quoting with sparse taker-only dislocation entries.
- Focus strikes with best signal quality and enforce spread/liquidity gates.
"""
...
class Trader:
    ...
    VEV_TAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 9, 5100: 6, 5200: 4}
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool) -> List[Order]:
        ...
        # Sparse execution: only top dislocation to avoid noisy overtrading.
