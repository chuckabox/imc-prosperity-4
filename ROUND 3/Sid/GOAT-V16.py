"""GOAT V16 - V14 restored (V15 regression reverted)

V15 tried to "fix" HYDROGEL by replacing the fixed anchor with adaptive
slow-EMA + a mid-clamp on quote prices. Result: HP module went from
~$2-3k/day (V14) to $0/day (V15). Total: V14 = $11.7k, V15 = $8k.

What V15 misread:
- The intraday MTM dips of -$2k I saw in V14 logs were NOT realized
  losses — they were inventory MTM during the price up-spike.
- V14's "anchor bias" (fair pulled below mid) was actually a profitable
  spread-capture feature: market takers lifted V14's wider asks at higher
  prices, generating consistent edge per fill.
- V15's mid-clamp tightened the quotes so much that no market takers
  crossed them -> zero fills -> zero PnL.

V16 = V14 byte-for-byte. Lesson: do not fix what is profitable.

Strategy (Ken's vfe gold + V14's ITM addition):
- HP anchor-blended EWMA (0.8 * ewma + 0.2 * 9991)
- Greeks-aware dynamic hedging (delta, gamma, vega, theta)
- Smile-fit pair trading on 5000/5100 vs 5200/5300
- Phase2 theta-aware exits
- VFE micro-structure fair + speed limiting
- Smile-based passive MM on 5200/5300/5400/5500
- Deep-ITM intrinsic MM on VEV_4000 and VEV_4500
"""
...
class Trader:
    ...
    # Greek Generation Parameters (Ken's exact)
    ...
    # Smile-Based Passive MM (Ken's exact)
    SMM_ENABLE = True
    ...
    # NEW: Deep-ITM intrinsic capture
    ITM_ENABLE = True
    ITM_STRIKES = [4000, 4500]
    ITM_EDGE = 1            # Min ticks above intrinsic to take/quote
    ITM_QUOTE_SIZE = 18
    ITM_POS_CAP = 60
    ...
    def _vev_itm(self, state: TradingState, S: Optional[float]) -> List[Order]:
        """Deep-ITM intrinsic-value MM for 4000/4500.

        These strikes are too deep ITM to be priced by the smile fit
        (their IV solver returns None because price <= intrinsic).
        They trade ~ S - K with a small premium. We capture wide-spread
        edge by quoting a tick above intrinsic on each side and taking
        any cross-spread mispricing.
        """
        ...
        for k in self.ITM_STRIKES:
            ...
            # Take if ask is at-or-below intrinsic (free money)
            if ba <= intrinsic and pos < self.ITM_POS_CAP:
                ...
                if qty > 0:
                    ...
                    pos += qty
            # Take if bid is well above intrinsic + edge
            ...
            # Quote inside the wide spread: bid at intrinsic, ask at intrinsic + edge + 1
