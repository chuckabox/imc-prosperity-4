...
- Includes v28 profitable parameter tweaks
- Includes lightweight v30 soft safety (open-phase + position-speed limiter)

No local module imports except datamodel (single-file submission friendly).
"""
...
class Trader:
    ...
    HP_SPEED_TRIGGER = 18
    VFE_SPEED_TRIGGER = 12
    SPEED_COOLDOWN_TS = 40_000
    OPEN_SCALE_MULT = 0.7
    SPEED_SCALE_MULT = 0.9
    ...
    def _vfe_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        maker_max = max(4, int(22 * local_scale))
