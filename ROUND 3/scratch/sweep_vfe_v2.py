"""sweep_vfe_v2.py — sweep VFE params, HP fixed at best."""
...
def _load_trader_module():
    ...
    spec = importlib.util.spec_from_file_location("user_trader1_vfe2", TRADER_PATH)
...
def main():
    ...
    grid = {
        "VFE_EWMA_ALPHA":     [0.10, 0.20, 0.35],
        "VFE_TAKE_EDGE":      [1, 2, 3],
        "VFE_NEUTRAL_FRONT":  [8, 15, 25],
        "VFE_TREND_THRESHOLD":[0.03, 0.05, 0.10],
    }
    ...
    print(f"Sweeping {len(combos)} VFE combos x {len(DAYS)} days", flush=True)
    ...
    for i, params in enumerate(combos):
        ...
        p_str = " ".join(f"{k.replace('VFE_','')}={v}" for k, v in params.items())
        print(f"  [{i+1:>3}/{len(combos)}] {p_str} | d0={per_day[0]:+.0f} d1={per_day[1]:+.0f} "
              f"T={total:+.0f} (eta {eta:.0f}s)", flush=True)
