"""trader_ken_v55_dual_allocator.py
...
- Allocator: dynamically scales agent budgets by regime (vol, spread, inventory pressure).
"""
...
class Trader:
    ...
    def _allocator(self, state: TradingState, vfe_mid: Optional[float]) -> Tuple[float, float]:
        ...
        # Budget split
        hydro_scale = risk_scale * (0.85 if opt_on else 1.00)
        opt_scale = risk_scale * (1.00 if opt_on else 0.35)
        return hydro_scale, opt_scale
