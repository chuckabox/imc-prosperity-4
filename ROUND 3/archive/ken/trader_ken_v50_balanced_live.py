"""trader_ken_v50_balanced_live.py

Balanced live-aware variant:
- Start from v41 standalone (high alpha baseline).
- Add only light Hydro taker safeguards (spread gate + short cooldown).
"""
...
class Trader:
    ...
    HP_MAKER_EDGE = 2.3
    HP_TAKER_MAX = 20
    HP_TAKER_SPREAD_MAX = 14
    HP_TAKER_COOLDOWN_TS = 500
    ...
    def _hydrogel_logic(self, state: TradingState, scale: float, risk_off: bool) -> Tuple[List[Order], Optional[float]]:
        ...
        spread = ba - bb
        now = int(state.timestamp)
        taker_allowed = taker_enabled and spread <= self.HP_TAKER_SPREAD_MAX and now >= int(self.history.get("hp_taker_until", -1))

        if taker_allowed and ba <= fair - self.HP_TAKER_EDGE and pos < lim:
        ...
        if taker_allowed and bb >= fair + self.HP_TAKER_EDGE and pos > -lim:
