"""trader_ken_v70_stacked_alpha.py
...
- Adds orthogonal alpha layers: HP imbalance and VFE microprice tilt.
"""
...
class Trader:
    ...
    VEV_TAKER_MAX_BY_STRIKE: Dict[int, int] = {5000: 5, 5100: 6, 5200: 9, 5300: 9, 5400: 5}
    VEV_REL_Z_ENTRY = 0.95
    VEV_PAIR_BASE_Q = 3
    ...
    def _vev_logic(self, state: TradingState, vfe_mid: float, scale: float, risk_off: bool) -> List[Order]:
        ...
        if rich and cheap:
            ...
            if sell_k != buy_k:
                ...
                sell_mx = max(self.VEV_PAIR_BASE_Q, int(self.VEV_TAKER_MAX_BY_STRIKE[sell_k] * scale))
                buy_mx = max(self.VEV_PAIR_BASE_Q, int(self.VEV_TAKER_MAX_BY_STRIKE[buy_k] * scale))
