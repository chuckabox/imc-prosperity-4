...
class Trader:
    ...
    def _update_mark_signal(self, state: TradingState, mem: Dict) -> None:
        ...
        for symbol, trades in state.market_trades.items():
            ...
            if abs(s) > 0.01: ms[symbol] = max(-40.0, min(40.0, s))
    ...
    def _take_edge_orders(self, symbol: str, state: TradingState, fair: float, threshold: float, max_take: int) -> List[Order]:
        if not self.CFG.get("enable_take", True): return []
        depth = state.order_depths.get(symbol)
        if not depth: return []
        orders: List[Order] = []
        pos = state.position.get(symbol, 0); lim = self.POSITION_LIMITS.get(symbol, 20)
    ...
    def _vev_orders(self, symbol: str, state: TradingState, vfe_mid: float) -> List[Order]:
        ...
        k = int(symbol.split("_")[1]); enabled = set(self.CFG.get("vev_strikes", [4000, 4500]))
        ...
        buy_cap = max(0, lim - pos); sell_cap = max(0, lim + pos)
        th = self.CFG.get("vev_threshold", 7.5); size = self.CFG.get("vev_size", 2)
    ...
    def run(self, state: TradingState):
        ...
        if hydro_mid is not None:
            ...
            skew = 0.06 * mem["mark_signal"].get("HYDROGEL_PACK", 0.0) + self.CFG.get("hydro_momo_k", -0.35) * momo
            result["HYDROGEL_PACK"].extend(self._mm_orders("HYDROGEL_PACK", state, hydro_mid, self.CFG.get("hydro_edge", 5), self.CFG.get("hydro_clip", 9), skew))
            result["HYDROGEL_PACK"].extend(self._take_edge_orders("HYDROGEL_PACK", state, hydro_mid + skew, self.CFG.get("hydro_take_th", 1.8), self.CFG.get("hydro_take_size", 6)))
        ...
        if vfe_mid is not None:
            ...
            skew = 0.05 * mem["mark_signal"].get("VELVETFRUIT_EXTRACT", 0.0) + self.CFG.get("vfe_momo_k", -0.25) * momo
            result["VELVETFRUIT_EXTRACT"].extend(self._mm_orders("VELVETFRUIT_EXTRACT", state, vfe_mid, self.CFG.get("vfe_edge", 2), self.CFG.get("vfe_clip", 9), skew))
            result["VELVETFRUIT_EXTRACT"].extend(self._take_edge_orders("VELVETFRUIT_EXTRACT", state, vfe_mid + skew, self.CFG.get("vfe_take_th", 1.3), self.CFG.get("vfe_take_size", 6)))
