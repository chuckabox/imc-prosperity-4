...
class Trader:
    ...
    def _vev_orders(self, symbol: str, state: TradingState, vfe_mid: float) -> List[Order]:
        ...
        k = int(symbol.split("_")[1]); enabled = set(self.CFG.get("vev_strikes", [4000, 4500, 5000, 5400, 5500]))
        ...
        buy_cap = max(0, lim - pos); sell_cap = max(0, lim + pos)
        th = self.CFG.get("vev_threshold", 6.8); size = self.CFG.get("vev_size", 3)
    ...
    def run(self, state: TradingState):
        ...
        if hydro_mid is not None:
            ...
            skew = 0.06 * mem["mark_signal"].get("HYDROGEL_PACK", 0.0) + self.CFG.get("hydro_momo_k", 0.20) * momo
            result["HYDROGEL_PACK"].extend(self._mm_orders("HYDROGEL_PACK", state, hydro_mid, self.CFG.get("hydro_edge", 6), self.CFG.get("hydro_clip", 8), skew))
            result["HYDROGEL_PACK"].extend(self._take_edge_orders("HYDROGEL_PACK", state, hydro_mid + skew, self.CFG.get("hydro_take_th", 2.0), self.CFG.get("hydro_take_size", 6)))
        ...
        if vfe_mid is not None:
            ...
            skew = 0.05 * mem["mark_signal"].get("VELVETFRUIT_EXTRACT", 0.0) + self.CFG.get("vfe_momo_k", 0.20) * momo
            result["VELVETFRUIT_EXTRACT"].extend(self._mm_orders("VELVETFRUIT_EXTRACT", state, vfe_mid, self.CFG.get("vfe_edge", 2), self.CFG.get("vfe_clip", 8), skew))
            result["VELVETFRUIT_EXTRACT"].extend(self._take_edge_orders("VELVETFRUIT_EXTRACT", state, vfe_mid + skew, self.CFG.get("vfe_take_th", 1.4), self.CFG.get("vfe_take_size", 6)))
